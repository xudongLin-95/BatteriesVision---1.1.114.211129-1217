...
BOOL CStorage::WriteSpotInspectionFile(C_MergeWork& tOutput)
{
	...
	strFilePath.Format("%s\\点检数据\\相机点检数据_%04d%02d%02d.csv", m_strMapFolderPath, m_tSaveTime.wYear, m_tSaveTime.wMonth, m_tSaveTime.wDay);
	HTuple hvExist;
	...
	if (hvExist == 0)
	{
		...
		{
			CString strTmp;
			strTmp.Format("%s面左点检宽(mm),%s面左点检高(mm),%s面右点检宽(mm),%s面右点检高(mm),%s面点检距离(mm),",
	...
	for (int nBWorkSN = 0; nBWorkSN < tOutput.vecB.size(); nBWorkSN++)
	{
		if (tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width.Length() == 5)
		{
			for (int j = 0; j < tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width.Length(); j++)
			{
				...
				{
					strTmp.Format("%0.4f,", tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[j].D());
				}
				else
				{
					strTmp = ",";
...
void CStorage::WriteRecordFileWet(const CString& strFilePath, C_MergeWork& tOutput)
{
	...
	if (CGlobal::SN_Tail == theGlobal.GetSystemSet().eSN)//机尾都是干膜 不需要记录湿膜的尺寸
	{
		return;
	}
