...
CTitleStatic::CTitleStatic()
{
	...
	m_rgbTitleText    = ::GetSysColor(COLOR_WINDOWTEXT);
	m_rgbBkgnd		  = RGB(58,58,58);//::GetSysColor(COLOR_BTNFACE);
	m_rgbTitleBkgnd   = RGB(48,48,48);//::GetSysColor(COLOR_BTNFACE);
...
CTitleStatic::~CTitleStatic()
{
	//TRACE(_T("in CTitleStatic::~CTitleStatic\n"));
...
/////////////////////////////////////////////////////////////////////////////////
//// PreSubclassWindow
void CTitleStatic::PreSubclassWindow()
{
	//TRACE(_T("in CTitleStatic::PreSubclassWindow\n"));
...
void CTitleStatic::SetMoveable(BOOL moveAble)
{
...
///////////////////////////////////////////////////////////////////////////////
// SetBackgroundColor
void CTitleStatic::SetBkColor(COLORREF rgbBkgnd, COLORREF rgbBkgndHigh, BackFillMode mode)
{
...
/////////////////////////////////////////////////////////////////////////////////
//// SetTransparent
void CTitleStatic::SetTransparent(BOOL bTranspMode)
{
...
void CTitleStatic::SetTextColor(COLORREF col)
{
	m_rgbText = col;
	RedrawWindow();
}

void CTitleStatic::SetTextColorAll(COLORREF colTitle,COLORREF colText)
{
	m_rgbTitleText = colTitle;
	m_rgbText = colText;
	RedrawWindow();
}

void CTitleStatic::SetAutoAdjustFont(BOOL bAutoAdjustFont)
{
	m_bAutoAdjustFont = bAutoAdjustFont;
}

void CTitleStatic::SetRounded(BOOL bRounded){
	m_bRounded = bRounded;
}
...
//void CTitleStatic::ReconstructFont()
//{
...
void CTitleStatic::SetFont(const CString& strFont,int nPointSize, int nWeight,
					   BOOL bRedraw /*= TRUE*/)
{
	LOGFONT lf;
	memset(&lf, 0, sizeof(LOGFONT));       // Zero out the structure.

	_tcscpy_s(lf.lfFaceName,strFont);			// Set Font Familly
...
void CTitleStatic::SetFontAll(const CString& strTitleFont, int nTitlePointSize,int nTitleWeight,const CString& strTextFont, int nTextPointSize,int nTextWeight, BOOL bRedraw)
{
	m_fontTitle.DeleteObject();
	VERIFY(m_fontTitle.CreateFontA(nTitlePointSize,0,0,0,nTitleWeight,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH | FF_SWISS,strTitleFont));

	LOGFONT lf;
	memset(&lf, 0, sizeof(LOGFONT));       // Zero out the structure.
	_tcscpy_s(lf.lfFaceName,strTextFont);			// Set Font Familly
	lf.lfHeight = nTextPointSize;// Set Height
	lf.lfWeight = nTextWeight;					// Set Weight
	SetFont(&lf, bRedraw);
}

void CTitleStatic::SetFont(LOGFONT *pLogFont, BOOL bRedraw /*= TRUE*/)
{
	ASSERT(pLogFont);
	if (!pLogFont)
		return;
...
void CTitleStatic::SetFont(CFont *pFont, BOOL bRedraw /*= TRUE*/)
{
	ASSERT(pFont);
	if (!pFont)
		return;
...
LRESULT CTitleStatic::OnSetFont(WPARAM wParam, LPARAM lParam)
{
...
void CTitleStatic::SetFont3D(BOOL bFont3D, Type3D type)
{
	m_bFont3d = bFont3D;
	m_3dType = type;
...
LRESULT CTitleStatic::OnSetText(WPARAM wParam, LPARAM lParam)
{
	LRESULT lrReturn(Default());
...
BOOL CTitleStatic::SetBitmap(HBITMAP hBitmap, ImageSize Emode, COLORREF rgbTransparent)
{
	m_bBitmap = TRUE;
	::DeleteObject(m_hBitmap);
	m_hBitmap = hBitmap;
...
BOOL CTitleStatic::SetBitmap(UINT nIDResource, ImageSize Emode, COLORREF rgbTransparent)
{
	...
	return CTitleStatic::SetBitmap(hBmp, Emode, rgbTransparent);
}

BOOL CTitleStatic::SetBitmap(LPCTSTR lpszResourceName, ImageSize Emode, COLORREF rgbTransparent)
{
	m_nResourceID = -1;
...
//void CTitleStatic::BlinkBmp(UINT nIDBmp1, UINT nIDBmp2, UINT Timeout)
//{
//	SetTimer(0, Timeout);
//
//
...
///////////////////////////////////////////////////////////////////////////////
// GetFontPointSize()
int CTitleStatic::GetFontPointSize(int nHeight)
{
...
///////////////////////////////////////////////////////////////////////////////
// GetFontHeight()
int CTitleStatic::GetFontHeight(int nPointSize)
{
	HDC hdc = ::CreateDC(_T("DISPLAY"), NULL, NULL, NULL);
	ASSERT(hdc);
	int cyPixelsPerInch = ::GetDeviceCaps(hdc, LOGPIXELSY);
...
void CTitleStatic::SetWindowText( LPCTSTR strText)
{
	m_strText = strText;
...
void CTitleStatic::SetTitleText(LPCTSTR strText)
{
	m_strTitleText = strText;
	RedrawWindow();
}

void CTitleStatic::Format(LPCTSTR szFmt, ...)
{
	ULONG	bytesWriten;
	TCHAR	szBuffer[2048];
	va_list args;
...
void CTitleStatic::AppendText(LPCTSTR szFmt, ...)
{
	ULONG	bytesWriten;
	TCHAR	szBuffer[2048];
	va_list args;
...
BOOL CTitleStatic::RedrawWindow()
{
	/*CRect rect;
...
void CTitleStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CMyMemDC MemDC(pDC, &rect, m_bTransparentBk);
	CSize sz = MemDC.GetTextExtent(m_strTitleText);
	CRect rtTitle(rect),rtText(rect);
	if (m_bHorizontal)
	{
		m_nTitleSize = max(m_nTitleSize, sz.cx + 22);
		rtTitle.right = rect.left + m_nTitleSize;
		rtText.left = rtTitle.right;
	}
	else
	{
		rtTitle.bottom = rect.top + m_nTitleSize;
		rtText.top = rtTitle.bottom;
	}
	...
	if ( !m_bBitmap){
		if ( !m_bTransparentBk ){
			if (m_fillmode == Normal)
			{
				if (m_bHover)
				{
					pDC->SetBkColor(m_crHiColor);
					MemDC.FillRect(&rect, m_pBrush);
				}
				else
				{
					//COLORREF crOld = pDC->SetBkColor(m_rgbTitleBkgnd);
					m_pBrush->DeleteObject();
					m_pBrush->CreateSolidBrush(m_rgbTitleBkgnd);
					//MemDC->SetBkColor(m_rgbTitleBkgnd);
					MemDC.FillRect(&rtTitle, m_pBrush);
					//pDC->SetBkColor(m_rgbBkgnd);
					//MemDC->SetBkColor(m_rgbBkgnd);
					m_pBrush->DeleteObject();
					m_pBrush->CreateSolidBrush(m_rgbBkgnd);
					MemDC.FillRect(&rtText, m_pBrush);
				}
			}
			else
			{
	...
	COLORREF rgbTitle = MemDC.SetTextColor(m_rgbTitleText);
	DrawText(&MemDC, &rtTitle, m_strTitleText);
	MemDC.SelectObject(pOldFont);
	MemDC.SetTextColor(rgbTitle);

	pOldFont  = MemDC.SelectObject( &m_font );
	COLORREF rgbText = MemDC.SetTextColor(m_rgbText);
	DrawText(&MemDC, &rtText, m_strText);
	MemDC.SelectObject(pOldFont);
...
void CTitleStatic::DrawText(CDC* pDCMem, CRect* pRect, CString csText)
{
...
#define NMAP_STYLE(src, dest) if(!(dwStyle & (src))) dwFlags |= (dest)
	...
	if (dwStyle & SS_CENTERIMAGE)
	{
		CRect rtCli;
		GetClientRect(rtCli);
		if (abs(rcCli.top - rtCli.top) <= 3)
		{
			if (m_bHorizontal)
			{
				rcCli.top = pRect->top + nDiff/2;
				newRC.left = pRect->left + 16;
			}
			else
			{
				rcCli.top = pRect->top + nDiff*2/3;
			}
		}
		else
		{
	...
	if (m_bAutoAdjustFont){
		if (nDiff < 0)
		{
			...
			SetFont( m_lf.lfFaceName, (pRect->bottom - pRect->top-2), m_lf.lfWeight );
			rcCli.top = pRect->top+1;
		}
	...
	{
		if (nDiff < 0){
			rcCli.top = pRect->top+2;
		}
	}
	
	for (int j = 0; j < nLine; j++){
		newRC.top = rcCli.top + tag.tmHeight * j;
		pDCMem->DrawTextA(asText[j], &newRC,dwFlags);
		if (m_bFont3d){
			if (m_3dType == Raised)
...
BOOL CTitleStatic::IsASingleWord(const CString & csText)
{
	char	cEnd = 0;
...
void CTitleStatic::DrawBitmap(CDC* pDCMem, CRect* pRect)
{
	CDC dcMem;
	CRect rect;
    GetClientRect( rect );
...
void CTitleStatic::DrawGradientFill(CDC* pDCMem, CRect* pRect, BackFillMode FillMode)
{
...
///////////////////////////////////////////////////////////////////////////////
// OnEraseBkgnd
BOOL CTitleStatic::OnEraseBkgnd(CDC* pDC)
{
...
void CTitleStatic::OnSize(UINT nType, int cx, int cy)
{
...
//HBRUSH CTitleStatic::CtlColor(CDC* pDC, UINT nCtlColor)
//{
...
void CTitleStatic::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CTitleStatic::OnMouseHover(WPARAM wparam, LPARAM lparam)
{
	m_bHover=TRUE;
...
LRESULT CTitleStatic::OnMouseLeave(WPARAM wparam, LPARAM lparam)
{
	m_bTracking = FALSE;
	m_bHover=FALSE;
	Invalidate();
...
void CTitleStatic::OnDropFiles(HDROP hDropInfo)
{
	int		nFiles = 0;
...
void CTitleStatic::OnRButtonDown(UINT nFlags, CPoint point)
{
