...
BOOL CLanguage::ReadParam()
{
	...
	strSystemFile.Format("%s//Config//System.vis", GetCurrentAppPathA());
	SetLanguage(GetPrivateProfileInt("Language", "Type", Chinese, strSystemFile));
...
CString CLanguage::GetCurrentAppPathA()
{
	CString path;
...
BOOL CLanguage::SetLanguage(int nTypeSN)
{
	...
	CString strPath;
	strPath.Format("%s//Config//Public//Language//", GetCurrentAppPathA());
	if (Chinese == nTypeSN)
	{
		strPath += "CN.la";
	}
	else if (English == nTypeSN)
	{
		strPath += "EN.la";
	}
	else
...
void CLanguage::MultMessagebox(CString strCN, CString strEN, CString strTC)
{
	if (Chinese == m_eLanguageType)
	{
		BCGPMessageBox(strCN);
		/*AfxMessageBox(strCN);*/
	}
	else if (English == m_eLanguageType)
	{
		/*MessageBoxEx(NULL, TEXT(strEN),TEXT("Warning"),  MB_YESNO, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US));*/
		//BCGPMessageBoxEx(NULL, TEXT(strEN), TEXT("Warning"), NULL, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US));
		BCGPMessageBox(strEN);
	}
	else if (TraditionalChinese == m_eLanguageType)
	{
		MessageBoxEx(NULL, TEXT(strTC), TEXT("Warning"), MB_OKCANCEL, MAKELANGID(LANG_CHINESE_TRADITIONAL, SUBLANG_CHINESE_TRADITIONAL));
	}
}
