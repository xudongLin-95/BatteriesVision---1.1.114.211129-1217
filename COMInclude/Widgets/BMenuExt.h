...
public:
	CBMenuExtData () {menuIconNormal=-1;xoffset=-1;bitmap=NULL;pContext=NULL;
...
//struct CBMenuItemInfo : public MENUITEMINFO {
struct CBMenuItemInfo : public 
//MENUITEMINFO 
...
class W_LIB CBMenuExt : public CMenu
{
	DECLARE_DYNAMIC( CBMenuExt )
public:
	CBMenuExt(); 
	virtual ~CBMenuExt();
	...
	CBMenuExt* AppendODPopupMenuA(LPCSTR lpstrText);
	CBMenuExt* AppendODPopupMenuW(wchar_t *lpstrText);
	...
	CBMenuExt* GetSubBCMenu(char* lpszSubMenuName);
	CBMenuExt* GetSubBCMenu(wchar_t* lpszSubMenuName);
	...
	static void SetMenuDrawMode(UINT mode){
		CBMenuExt::original_drawmode=mode;
		CBMenuExt::xp_drawmode=mode;
	};
	...
	static void SetSelectDisableMode(BOOL mode){
		CBMenuExt::original_select_disabled=mode;
		CBMenuExt::xp_select_disabled=mode;
	};
	static int CBMenuExt::GetMenuDrawMode(void);
	static BOOL CBMenuExt::GetSelectDisableMode(void);

	// how the bitmaps are drawn in XP Luna mode
	static void SetXPBitmap3D(BOOL val){
		CBMenuExt::xp_draw_3D_bitmaps=val;
	};
	static BOOL GetXPBitmap3D(void){return CBMenuExt::xp_draw_3D_bitmaps;}
...
protected:
	...
	CBMenuExtData *CBMenuExt::FindMenuItem(UINT nID);
	CBMenuExt *FindMenuOption(int nId,int& nLoc);
	CBMenuExt *FindAnotherMenuOption(int nId,int& nLoc,CArray<CBMenuExt*,CBMenuExt*>&bcsubs,
								  CArray<int,int&>&bclocs);
	CBMenuExtData *FindMenuOption(wchar_t *lpstrText);
	void InsertSpaces(void);
	void DrawCheckMark(CDC *pDC,int x,int y,COLORREF color);
	void DrawRadioDot(CDC *pDC,int x,int y,COLORREF color);
	CBMenuExtData *NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string);
	void SynchronizeMenu(void);
	void CBMenuExt::InitializeMenuList(int value);
	void CBMenuExt::DeleteMenuList(void);
	CBMenuExtData *CBMenuExt::FindMenuList(UINT nID);
...
// Member Variables
protected:
	CTypedPtrArray<CPtrArray, CBMenuExtData*> m_MenuList;  // Stores list of menu items 
...
class CBMenuToolBar : public CToolBar{
public:
	CBMenuToolBar() : CToolBar() {m_iconX=m_iconY=0;}
...
class CBMenuMemDC : public CDC
{
public:

    // constructor sets up the memory DC
    CBMenuMemDC(CDC* pDC,LPCRECT lpSrcRect) : CDC()
    {
        ASSERT(pDC != NULL);
    ...
    CBMenuMemDC* operator->() {return this;}
        
    // Allow usage as a pointer
    operator CBMenuMemDC*() {return this;}

private:
    CBitmap  m_bitmap;      // Offscreen bitmap
    CBitmap* m_pOldBitmap;  // bitmap originally found in BCBMenuMemDC
