...
void FullScreen(HWND hWnd)
{
	...
	rectDeskTop.top = 0;
	rectDeskTop.left = 0;
	rectDeskTop.right = GetSystemMetrics(SM_CXSCREEN); //获取屏幕水平分辨率
	rectDeskTop.bottom = GetSystemMetrics(SM_CYSCREEN); //获取屏幕垂直分辨率
	::AdjustWindowRectEx(rectDeskTop, CWnd::FromHandle(hWnd)->GetStyle(), FALSE, CWnd::FromHandle(hWnd)->GetExStyle());
	wpnew.showCmd = SW_SHOWNORMAL;
	wpnew.rcNormalPosition = rectDeskTop;
	SetWindowPlacement(hWnd, &wpnew);//指定窗口显示状态和恢复，最大化，最小化位置
}

void FullScreenWithTaskbar(HWND hWnd)
{
	CRect rectDeskTop;
	WINDOWPLACEMENT wpnew;
	GetWindowPlacement(hWnd,&wpnew);
...
{
	static LONG s_nSrcX(1366), s_nSrcY(768);
	CRect rc;
	...
	//得到调整后的屏幕尺寸
	LONG nDstX = rc.Width();
	LONG nDstY = rc.Height();

	float fsp[2];
	fsp[0] = (float)nDstX / s_nSrcX;
	fsp[1] = (float)nDstY / s_nSrcY;
...
{
	AdaptiveSize(hWnd, FALSE);
	FullScreen(hWnd);
	AdaptiveSize(hWnd, TRUE);
}

void FullScreenTaskbarAdaptive(HWND hWnd)
{
	AdaptiveSize(hWnd, FALSE);
	FullScreenWithTaskbar(hWnd);
	AdaptiveSize(hWnd, TRUE);
}

void OutputLogFunction(LOGCALLBACK PLog,int& nGraberSN, CString strRemindID,CString strFuntionPart,HTuple& hvStartTime,HTuple& hvEndTime)//输出时间日志
{
	CString strTimeLog;
	strTimeLog.Format("Camera%d-%s_%s_%.2fms",nGraberSN,strRemindID,strFuntionPart,(hvEndTime.D()-hvStartTime.D())*1000);
	//strTimeLog.Format("Camera%d-%s_%s_%.2fms",m_tInit.nGrabberSN,strRemindID,strFuntionPart,(hvEndTime.D()-hvStartTime.D())*1000);
	/*m_tInit.pLog(strTimeLog,StdLog,NoExcp);*/
	PLog(strTimeLog,DebugLog,NoExcp);
}
void OutputLogFunction(LOGCALLBACK PLog,int& nGraberSN,CString strErrorID,CString strFuntionPart,CString strErrorProcName,CString strErrorContent)//输出错误停机日志
{
	...
	strException.Format("Camera%d-%s_%s_%s_%s",nGraberSN,strErrorID,strFuntionPart,strErrorProcName,strErrorContent);
	PLog(strException,ExcpLog,FatalExcp);
