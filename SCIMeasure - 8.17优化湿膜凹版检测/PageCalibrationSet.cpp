...
{
	ePOWERLIMITMODULE eP = PM_Manage;
	BOOL bEnable = m_pOwners->m_tInit.pMain((eModuleType)-1, "CheckPermission", "", &eP, NULL);
	...
	if (!bEnable)
	{
		GetDlgItem(IDC_PARAM_KVALUE)->EnableWindow(FALSE);
		GetDlgItem(IDC_PARAM_KVALUE2)->EnableWindow(FALSE);
	}
