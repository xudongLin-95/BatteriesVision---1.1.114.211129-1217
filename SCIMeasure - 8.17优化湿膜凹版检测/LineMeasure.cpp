...
#pragma region "先对拟合方式进行判断，如果不是正常拟合直接就返回了"
		...
		{
			TempFitLinePara.dStartRow = Height.I() / 8 * 5;
			TempFitLinePara.dEndRow = Height.I() / 8 * 3;
		}
		else
		{
			TempFitLinePara.dStartRow = Height.I() / 8 * 3;
			TempFitLinePara.dEndRow = Height.I() / 8 * 5;
		}
...
#pragma endregion 
		...
		try
		{
			...
			if (RowBegin.Length() > 0)
			{
				...
				//找边
				if (PRE_BaseCoatWet == FitLinePara.ePreproc
					&& (LT_BaseCoatCoat == eType || LT_CoatBaseCoat == eType))
				{
					...
					HTuple hvVer, hvHor, hvUsedThreshold, hvMean, hvInc;
					int WIDTH = 40;
					int HEIGHT = 30;
					GenRectangle1(&hoROIRegion, Height.I() / 2 - HEIGHT, (ColBegin[0].D() + ColEnd[0].D()) / 2 - WIDTH,
						Height.I() / 2 + HEIGHT, (ColBegin[0].D() + ColEnd[0].D()) / 2 + WIDTH);
					ReduceDomain(Image, hoROIRegion, &hoReduce);
					CropDomain(hoReduce, &hoReduce);
					ScaleImage(hoReduce, &hoReduceScaled, 3, 0);
					GrayClosingRect(hoReduceScaled, &hoGrayClosing, 11, 2);
					//BinaryThreshold(hoGrayClosing, &hoRegion, "max_separability", "dark", &hvUsedThreshold);
					VarThreshold(hoGrayClosing, &hoRegion, 50, 50, 0.3, 5, "dark");
					...
					SelectShape(hoRegion, &hoRegion, "height", "and", HEIGHT * 2 - 1, HEIGHT * 2 + 1);
					...
					hvCol += (ColBegin[0].D() + ColEnd[0].D()) / 2 - WIDTH;
...
BOOL CLineMeasure::AutoGenMeasureLine(const GenLineInput& tIn, GenLineOutput& tOut)
{
	...
	try
	{
		...
		if (m_tInit.tProductInfo.nPDType != ThreeCavity)
		{
			...
			{
				nLocalOffset = m_tInit.tProductInfo.tParam.vecBaseCoatWidth[0].dStandard / m_vecResolution[m_tInit.nSN].dColRes;
			}
			else if (m_tInit.tProductInfo.bExistAT11 && m_tInit.tProductInfo.tParam.vecAT11Width[0].dStandard != 0)
			{
				nLocalOffset = m_tInit.tProductInfo.tParam.vecAT11Width[0].dStandard / m_vecResolution[m_tInit.nSN].dColRes;
			}
			...
			for (int i = 0; i < hvLineCol.Length(); i++)
			{
				if (i == 0 || i == hvLineCol.Length() - 1)
					m_tMeasureParam.vecLineParam[i].dWidth = 1000;
				else
