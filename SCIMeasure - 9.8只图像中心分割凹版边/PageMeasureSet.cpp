...
void CPageMeasureSet::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBIndex(pDX, IDC_COMBO_MEASURE_PREPROC, m_Combo_nPreproc);
}
...
BEGIN_MESSAGE_MAP(CPageMeasureSet, CBCGPDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_MEASURE_PREPROC, &CPageMeasureSet::OnCbnSelchangeComboMeasurePreproc)
END_MESSAGE_MAP()
...
void CPageMeasureSet::RefreshUIData()
{
	...
	tParam.ePreproc = (CLineMeasure::ePreprocess)m_Combo_nPreproc;
...
void CPageMeasureSet::DispSearchLineParam(int nSN)
{
	...
	m_Combo_nPreproc = tParam.ePreproc;
...
void CPageMeasureSet::OnCbnSelchangeComboMeasurePreproc()
{
	// TODO: 在此添加控件通知处理程序代码
	RefreshUIData();
}
