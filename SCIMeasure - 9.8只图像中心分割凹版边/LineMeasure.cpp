...
#pragma endregion 
		...
		try
		{
				...
				&& (LT_BaseCoatCoat == eType || LT_CoatBaseCoat == eType))
			{
				...
				HTuple hvVer, hvHor, hvUsedThreshold, hvMean, hvInc, hvCoatGray;
				int WIDTH = 50;
				int HEIGHT = 300;
				GenRectangle1(&hoROIRegion, Height.I() / 2 - HEIGHT, ColBegin - WIDTH, Height.I() / 2 + HEIGHT, ColBegin + WIDTH);
				//获取涂布平均灰度
				if (LT_BaseCoatCoat == eType)
				{
					HObject hoCoatRegion;
					GenRectangle1(&hoCoatRegion, Height.I() / 2 - HEIGHT, ColBegin, Height.I() / 2 + HEIGHT, ColBegin + WIDTH);
					Intensity(hoCoatRegion, Image, &hvCoatGray, &HTuple());
				}
				else if (LT_CoatBaseCoat == eType)
				{
					HObject hoCoatRegion;
					GenRectangle1(&hoCoatRegion, Height.I() / 2 - HEIGHT, ColBegin - WIDTH, Height.I() / 2 + HEIGHT, ColBegin);
					Intensity(hoCoatRegion, Image, &hvCoatGray, &HTuple());
				}
				ReduceDomain(Image, hoROIRegion, &hoReduce);
				CropDomain(hoReduce, &hoReduce);
				double dScale = 255.0 / hvCoatGray.D();
				ScaleImage(hoReduce, &hoReduceScaled, dScale, 0);
				GrayClosingRect(hoReduceScaled, &hoGrayClosing, 5, 1);
				//BinaryThreshold(hoGrayClosing, &hoRegion, "max_separability", "dark", &hvUsedThreshold);
				VarThreshold(hoGrayClosing, &hoRegion, 50, 50, 0.3, FitLinePara.nThr, "dark");
				...
				Intensity(hoRegion, hoGrayClosing, &hvMean, &HTuple());
				if (hvMean.Length() > 0)
				{
					...
					if (hvInc.Length() > 0 && hvInc != -1)
					{
						...
						if (LT_BaseCoatCoat == eType)
						{
							SmallestRectangle1(hoBaseCoatRegion, &HTuple(), &HTuple(), &HTuple(), &hvCol);
							HObject hoLeftRegion, hoRightRegion;
							HTuple hvLeftMean, hvRightMean, hvLeftSig, hvRightSig;
							GenRectangle1(&hoLeftRegion, Height.I() / 2 - 20, hvCol + ColBegin - WIDTH - 5, Height.I() / 2 + 20, hvCol + ColBegin - WIDTH - 3);
							GenRectangle1(&hoRightRegion, Height.I() / 2 - 20, hvCol + ColBegin - WIDTH + 3, Height.I() / 2 + 20, hvCol + ColBegin - WIDTH + 5);
						...
						{
							SmallestRectangle1(hoBaseCoatRegion, &HTuple(), &hvCol, &HTuple(), &HTuple());
							HObject hoLeftRegion, hoRightRegion;
							HTuple hvLeftMean, hvRightMean, hvLeftSig, hvRightSig;
							GenRectangle1(&hoLeftRegion, Height.I() / 2 - 20, hvCol + ColBegin - WIDTH - 5, Height.I() / 2 + 20, hvCol + ColBegin - WIDTH - 3);
							GenRectangle1(&hoRightRegion, Height.I() / 2 - 20, hvCol + ColBegin - WIDTH + 3, Height.I() / 2 + 20, hvCol + ColBegin - WIDTH + 5);
			...
			if (PRE_PointFilter == FitLinePara.ePreproc)
			{
				...
				for (int i = 0; i < hvMeasureRow.Length(); i++)
				{
					...
					HObject hoLeftRegion, hoRightRegion;
					HTuple hvLeftMean, hvRightMean, hvLeftSig, hvRightSig;
					GenRectangle1(&hoLeftRegion, hvRowTmp - 20, hvColTmp - 10, hvRowTmp + 20, hvColTmp - 3);
					GenRectangle1(&hoRightRegion, hvRowTmp - 20, hvColTmp + 3, hvRowTmp + 20, hvColTmp + 10);
					Intensity(hoLeftRegion, Image, &hvLeftMean, &hvLeftSig);
					Intensity(hoRightRegion, Image, &hvRightMean, &hvRightSig);
	...
	const eLineType& eType, HTuple* hvEdgeCol)
{
	try
	{
		...
		{
			HObject hoCoatRegion;
			GenRectangle1(&hoCoatRegion, hvRow - HEIGHT, hvCol, hvRow + HEIGHT, hvCol + WIDTH);
			Intensity(hoCoatRegion, Image, &hvCoatGray, &HTuple());
		}
		else if (LT_CoatBaseCoat == eType)
		{
			HObject hoCoatRegion;
			GenRectangle1(&hoCoatRegion, hvRow - HEIGHT, hvCol - WIDTH, hvRow + HEIGHT, hvCol);
			Intensity(hoCoatRegion, Image, &hvCoatGray, &HTuple());
		}
		ReduceDomain(Image, hoROIRegion, &hoReduce);
		CropDomain(hoReduce, &hoReduce);
