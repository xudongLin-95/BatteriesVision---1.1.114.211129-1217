...
public:
	...
	BOOL Init();
	BOOL ReadParam();
	BOOL ReadSpotInspectionParam();
	BOOL WriteParam();
	BOOL WriteProductParam(const int& nSN);
	BOOL WriteSpotInspectionParam();
	...
	void SetCurrentProductInfo(const tagPDInfo tInfo) {
		for (int i = 0; i < m_vecProductList.size(); i++)
		{
			if (m_vecProductList[i].strPDName == tInfo.strPDName && i != m_nCurrentProductSN)
				return;
		}
	...
	void SetCurrentProductInfoTmp(const tagPDInfo& tInfo) {
		CLockHelper lk(&m_csLock);
		m_tCurrentPdInfo = tInfo;
	}
	...
	CString		GetCurrentProductName() { return m_strCurrentProductName; }//当前产品名称
	const tagPDInfo & GetCurrentProductInfo() { /*CLockHelper lk(&m_csLock);*/ return m_tCurrentPdInfo; }
...
public:
	...
	HTupleVector	m_hvcOffsetData;		//测量数据

private:
	CString m_strCurrentProductFolderPath;
	CString m_strCurrentProductName;//当前产品名称
	CString m_strCurrentProductParamPath;
	CString m_strSpotInspectionParamPath;//点检文件路径

	int		m_nProductCount;
	int		m_nCurrentProductSN;
	ProductList m_vecProductList;
	tagPDInfo	m_tCurrentPdInfo;
