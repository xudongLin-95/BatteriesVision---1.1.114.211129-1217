...
CPageDispList4::CPageDispList4(CWnd* pParent /*=NULL*/)
	...
	, m_EDIT_dFirstArticleIntervalTime(0)
	, m_EDIT_nFAIIngoreNum(0)
	, m_EDIT_nFAINum(0)
{
...
void CPageDispList4::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_FAI_TIME, m_EDIT_dFirstArticleIntervalTime);
	DDX_Text(pDX, IDC_EDIT_FAI_IGNORENUM, m_EDIT_nFAIIngoreNum);
	DDX_Text(pDX, IDC_EDIT_FAI_NUM, m_EDIT_nFAINum);
}
...
BEGIN_MESSAGE_MAP(CPageDispList4, CBCGPDialog)
	...
	ON_WM_PAINT()
	ON_EN_CHANGE(IDC_EDIT_FAI_IGNORENUM, &CPageDispList4::OnEnChangeEditFaiIgnorenum)
	ON_EN_CHANGE(IDC_EDIT_FAI_NUM, &CPageDispList4::OnEnChangeEditFaiNum)
	ON_EN_CHANGE(IDC_EDIT_FAI_TIME, &CPageDispList4::OnEnChangeEditFaiTime)
	ON_BN_CLICKED(IDC_BUTTON_FAI_COMPEN, &CPageDispList4::OnBnClickedButtonFaiCompen)
END_MESSAGE_MAP()
...
void CPageDispList4::InitUI()
{
	...
	m_EDIT_dFirstArticleIntervalTime = theGlobal.m_Judge.m_tFirstArticleInspParam.dIntervalTime;
	m_EDIT_nFAIIngoreNum = theGlobal.m_Judge.m_tFirstArticleInspParam.nStartIgnoreDataNum;
	m_EDIT_nFAINum = theGlobal.m_Judge.m_tFirstArticleInspParam.nCountinousDataNum;
	UpdateData(FALSE);
}
...
{
	if (!theGlobal.m_Judge.m_tFirstArticleInspCount.bStart) return;
...
{
	// TODO: 在此添加控件通知处理程序代码
	if (IDYES != theGlobal.m_Language.MessageboxQuestion("是否进行一键首件?", "Whether to FAI mode?", ""))
		return;
	...
	theGlobal.m_Judge.m_tFirstArticleInspCount.bStartIgnore = TRUE;
}
...
void CPageDispList4::OnEnChangeEditSpotinspectionTime()
{
	...
	// TODO:  在此添加控件通知处理程序代码
	UpdateData(TRUE);
	theGlobal.m_Judge.m_tFirstArticleInspParam.dIntervalTime = m_EDIT_dFirstArticleIntervalTime;
	theGlobal.m_Judge.WriteParam();
}

void CPageDispList4::UIPowerLimit()
{
	BOOL bEnable = theGlobal.m_Permission.CheckPermission(PM_Detect);
	GetDlgItem(IDC_EDIT_FAI_IGNORENUM)->EnableWindow(bEnable);
	GetDlgItem(IDC_EDIT_FAI_NUM)->EnableWindow(bEnable);
	GetDlgItem(IDC_EDIT_FAI_TIME)->EnableWindow(bEnable);
}

void CPageDispList4::OnPaint()
{
...
void CPageDispList4::OnEnChangeEditFaiIgnorenum()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CBCGPDialog::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
	...
	// TODO:  在此添加控件通知处理程序代码
	UpdateData(TRUE);
	theGlobal.m_Judge.m_tFirstArticleInspParam.nStartIgnoreDataNum = m_EDIT_nFAIIngoreNum;
	theGlobal.m_Judge.WriteParam();
}


void CPageDispList4::OnEnChangeEditFaiNum()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CBCGPDialog::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
	...
	// TODO:  在此添加控件通知处理程序代码
	UpdateData(TRUE);
	theGlobal.m_Judge.m_tFirstArticleInspParam.nCountinousDataNum = m_EDIT_nFAINum;
	theGlobal.m_Judge.WriteParam();
}


void CPageDispList4::OnEnChangeEditFaiTime()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CBCGPDialog::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CPageDispList4::OnBnClickedButtonFaiCompen()
{
	// TODO: 在此添加控件通知处理程序代码
	if (IDYES != theGlobal.m_Language.MessageboxQuestion("是否清零当前固定补偿值,进行自动计算补偿?", "Whether to auto calculate compensation for FAI?", ""))
		return;

	tagPDInfo tPdInfo = theGlobal.m_ProductTool.GetCurrentProductInfo();
	tagPDInfo tOldPdInfo = tPdInfo;
	...
	theGlobal.m_Judge.m_tFirstArticleInspCount.bStartCalculateCompen = TRUE;

	CString strLog;
	strLog.Format("当前固定补偿值清零,首件自动计算补偿值开始");
