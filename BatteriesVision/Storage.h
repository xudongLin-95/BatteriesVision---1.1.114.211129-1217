...
public:
	...
	CString  GetImageInquireFolderPath() { return m_strImageInquire; }
	//生成存储数据的路径
	...
	//写点检结果判定文件
	BOOL WriteSpotInspectionReport(const CJudge::tagSpotInspCount& tCount);
	//创建首件数据文件
	BOOL CreateFirstArticleInspReport(const CString& strFilePath);
	//写首件结果判定文件
	BOOL WriteFirstArticleInspReport(const CJudge::tagFirstArticleInspCount& tCount);
private:
	void InitQueues();
	void ReleaseQueues();
	BOOL InitThread();
	...
	//获取要写入文件的时间数据
	CString GetSaveTimeS();//时间格式1
	CString GetSaveTimeS2();//时间格式2
	CString GetSaveTimeS3();//时间格式3，不包含毫秒
	//保存数据
	...
	//写首件数据
	BOOL WriteFirstArticleInspFile(C_MergeWork& tOutput);
...
private:
	...
	CString           m_strImageInquire;                      //瑕疵图像查询路径
