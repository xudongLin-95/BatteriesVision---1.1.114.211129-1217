...
BEGIN_MESSAGE_MAP(CPageCommunicationSet, CBCGPDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_SET_BETARAY, &CPageCommunicationSet::OnBnClickedButtonSetBetaray)
END_MESSAGE_MAP()
...
void CPageCommunicationSet::OnBnClickedButtonSetBetaray()
{
	if (!theGlobal.m_pMainDlg->SendMessage(WM_CHECK_PMSN, PM_BetaRay, NULL)) return;
	if (!theGlobal.m_CommunicationTool.ShowParamDlg(CCommunicationTool::BetaRay))
	{
		theGlobal.m_Language.MessageboxError("打开测厚仪设置失败。", "Fail to open Beta-Ray settings.", "");
	}
}
