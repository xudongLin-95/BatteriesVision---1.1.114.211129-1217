...
BOOL CBatteriesVisionDlg::OnInitDialog()
{
	...
	theGlobal.InitUploadDlg();
	theGlobal.InitDefectImageDlg();
...
void CBatteriesVisionDlg::InitStatusBar()
{
	...
	m_wndStatusBar.SetPaneInfo(STATUS_BAR_CHECK, IDS_STRING_CHECk, SBPS_NORMAL, RectStatusBar.Width() / 9 * 2 / 4 - 20);
	m_wndStatusBar.SetPaneInfo(STATUS_BAR_PLC, IDS_STRING_PLC, SBPS_NORMAL, RectStatusBar.Width() / 9 * 2 / 4 - 20);
	m_wndStatusBar.SetPaneInfo(STATUS_BAR_IO, IDS_STRING_IO, SBPS_NORMAL, RectStatusBar.Width() / 9 * 2 / 4 - 20);
	m_wndStatusBar.SetPaneInfo(STATUS_BAR_RECTIFY, IDS_STRING_RECTIFY, SBPS_NORMAL, RectStatusBar.Width() / 9 * 2 / 4 - 20);
...
void CBatteriesVisionDlg::OnBnClickedButtonStart()
{
	if (theGlobal.IsStart())
	{
		...
		if (theGlobal.m_Permission.GetPermission()< PT_ADMINISTRATOR)
		{
			return;
		}
...
void CBatteriesVisionDlg::RefreshStatusBar()
{
	try
	{
		...
		//点检
		double nReCheckState = 0;
		nReCheckState = theGlobal.m_Judge.JudgeCheck();
		if (nReCheckState>0)
		{
			COLORREF col = clExcp;
			if (nReCheckState == 2)
			{
				strInfo.Format("[点检] 全OK");
				col = clNomal;
			}
			else if (nReCheckState == 1)
				strInfo.Format("[点检] 全NG");
			else if (nReCheckState == 2)
				strInfo.Format("[点检] 正NG");
			else
				strInfo.Format("[点检] 反NG");
			m_wndStatusBar.SetPaneText(STATUS_BAR_CHECK, strInfo);
			m_wndStatusBar.SetPaneBackgroundColor(STATUS_BAR_CHECK, col);
		}
		else
		{
			CString sReCheckState;
			strInfo.Format("[点检] %s", sReCheckState="点检未知");
			m_wndStatusBar.SetPaneText(STATUS_BAR_CHECK, strInfo);
			m_wndStatusBar.SetPaneBackgroundColor(STATUS_BAR_CHECK, clExcp);
		}
	...
	try
	{
		if (theGlobal.m_Permission.GetUserInfo().bLogin  && theGlobal.m_Permission.GetUserInfo().ePowerType > PT_OPERATOR)
		{
			...
			if (GetDiffSeconds(theGlobal.m_Permission.GetUserInfo().tLoginTime, nowSysTime) > theGlobal.m_Permission.m_nLoginDelayTime)//暂时没开放
			{
...
void CBatteriesVisionDlg::ExitDo()
{
	...
	theGlobal.CloseDefectImageDlg();
	theGlobal.CloseUploadDlg();
