...
int		WM_SPOTINSPECTION_REFRESH = RegisterWindowMessage(_T("WM_SPOTINSPECTION_REFRESH"));
CGlobal theGlobal;
CGlobal::CGlobal()
	:m_hovecHistroyImage(1)
{
	...
	//例如1.0.7.201020 Beta
	m_strVersion = "1.1.114.211220";
	try 
	{
		//SetSystem("database", "true");
		//ResetObjDb(100000, 100000, 1);
		SetSystem("width", 50000);
		SetSystem("height", 50000);
		...
		SetSystem("global_mem_cache", "shared");
...
void CGlobal::InitDefectImageDlg() {
	m_pDlgDefectImageDisp = new DlgDefctImageDisp;
	m_pDlgDefectImageDisp->Create(DlgDefctImageDisp::IDD);
	m_pDlgDefectImageDisp->CenterWindow();
	m_pDlgDefectImageDisp->ShowWindow(SW_HIDE);
	m_pDlgDefectImageDisp->UpdateWindow();
}

void CGlobal::InitUploadDlg()
{
	m_pDlgUploadAlarm = new DlgUploadAlarm;
	m_pDlgUploadAlarm->Create(DlgUploadAlarm::IDD);
	m_pDlgUploadAlarm->CenterWindow();
	m_pDlgUploadAlarm->ShowWindow(SW_HIDE);
	m_pDlgUploadAlarm->UpdateWindow();
}
...
void CGlobal::CloseDefectImageDlg()
{
	if (NULL == m_pDlgDefectImageDisp)
		return;
	m_pDlgDefectImageDisp->PostMessageA(WM_CLOSE);
	m_pDlgDefectImageDisp->ShowWindow(SW_HIDE);
}

void CGlobal::CloseUploadDlg()
{
	if (NULL == m_pDlgUploadAlarm)
		return;
	m_pDlgUploadAlarm->PostMessageA(WM_CLOSE);
	m_pDlgUploadAlarm->ShowWindow(SW_HIDE);
}
...
void CGlobal::ShowDefectImageDlg(C_MergeWork& tOutput)
{
	if (NULL == m_pDlgDefectImageDisp)
		return;

	m_pDlgDefectImageDisp->ShowAlarm(tOutput);
}

void CGlobal::ShowUploadDlg(const CString& strInfo)
{
	if (NULL == m_pDlgUploadAlarm || strInfo == "")
		return;

	m_pDlgUploadAlarm->ShowAlarm(strInfo);
}
...
BOOL CGlobal::PostMessageToSpotInspDlg(UINT message, WPARAM wParam, LPARAM lParam)
{
	if (m_pSpotInspection->GetSafeHwnd() != NULL
		&& IsWindow(m_pSpotInspection->GetSafeHwnd()))
		return m_pSpotInspection->SendMessageA(message, wParam, lParam);
