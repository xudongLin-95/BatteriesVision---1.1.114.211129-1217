...
extern int		WM_SPOTINSPECTION_REFRESH;//点检结果刷新
...
public:
	...
	CPageDispList3*		m_pSpotInspection;	//点检界面指针
...
public:
	...
	void CloseAlarmDlg();
	//瑕疵图像显示框
	void InitDefectImageDlg();
	void ShowDefectImageDlg(C_MergeWork& tOutput);
	void CloseDefectImageDlg();
	//通讯异常显示框
	void InitUploadDlg();
	void ShowUploadDlg(const CString& strInfo);
	void CloseUploadDlg();
	//提示对话框
	void InitTipDlg();
	...
	//给点检界面发消息
	BOOL PostMessageToSpotInspDlg(UINT message, WPARAM wParam = 0, LPARAM lParam = 0);
...
private:
	...
	DlgDefctImageDisp* m_pDlgDefectImageDisp;  //瑕疵图像对话框
	DlgUploadAlarm* m_pDlgUploadAlarm;//通讯异常对话框
