...
int WINAPI CCallback::MainCallback(eModuleType eType, const CString& strMessage, const CString& strSubMessage, void* wParam, void* lParam)
{
	...
	{
		CString strLog;
		strLog.Format("MainCallback: 调用主回调异常, %s(%s)", e.ErrorMessage().Text(), e.ProcName().Text());
...
int CCallback::SCIMeasureCall(const CString& strMessage, const CString& strSubMessage, void* wParam, void* lParam)
{
	if ("GetAutoCompensationEnable" == strMessage)
	{
		...
		{
			CString strLog;
			strLog.Format("CCallback_SCIMeasureCall: 干湿膜动态补偿禁用%s, 首件%s, 不使用动态补偿功能", theGlobal.m_Upload.m_bAutoCompensationDisable ? "开启" : "关闭",
				theGlobal.m_Judge.m_tFirstArticleInspCount.bStart ? "开启" : "关闭");
