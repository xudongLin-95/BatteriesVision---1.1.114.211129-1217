...
CStorage::~CStorage()
{
	...
	try
	{
		if (m_fileRecord.m_hFile != NULL)
			m_fileRecord.Close();
	}
	catch (CFileException&e) {}
	try
	{
		if (m_fileRecordAll.m_hFile != NULL)
			m_fileRecordAll.Close();
	}
	catch (CFileException&e) {}
	try
	{
		if (m_fileRecord_wet.m_hFile != NULL)
...
void CStorage::Enqueue(const int& nSN, const HTuple& MessageHandle, const HTuple& GenParamName, const HTuple& GenParamValue)
{
	...
	catch (HException& e)
	{
		...
		{
			SetMessageQueueParam(m_hvecStorageQueues[nSN].T(), "flush_queue", 1);
			CString strException;
			strException.Format("T_Merge%d保存队列已满,已清空队列", nSN);
...
void CStorage::InitQueues()
{
	for (int i = 0; i < m_nSaveThreadNum; i++)
	{
		...
		SetMessageQueueParam(m_hvecStorageQueues[i].T(), "max_message_num", 5);
	}
}
...
CString CStorage::GetSaveTimeS3()
{
	CString strTime;
	strTime.Format("%04d-%02d-%02d %02d:%02d:%02d", m_tSaveTime.wYear, m_tSaveTime.wMonth, m_tSaveTime.wDay,
		m_tSaveTime.wHour, m_tSaveTime.wMinute, m_tSaveTime.wSecond);
...
{
	::GetLocalTime(&m_tSaveTime);//保留，否则getsavetime()不更新
	/*::GetLocalTime(&m_tSaveTime);
	...
	m_strThumbImageSaveFolder.Format("%s\\涂布CCD图像_%s\\%s\\%04d%02d%02d\\%s_Defect\\", m_strMapFolderPath, theGlobal.GetSystemSet().strWorkshopName,
		theGlobal.m_ProductTool.GetCurrentProductName(), m_tSaveTime.wYear, m_tSaveTime.wMonth, m_tSaveTime.wDay, theGlobal.GetCurrentBatch());*/
	m_strDataSaveFolder.Format("%s\\涂布CCD原始数据_%s\\%s\\%s\\", m_strMapFolderPath, theGlobal.GetSystemSet().strWorkshopName,
		theGlobal.m_ProductTool.GetCurrentProductName(), theGlobal.GetCurrentBatch());

	m_strImageSaveFolder.Format("%s\\涂布CCD图像_%s\\%s\\%s\\", m_strMapFolderPath, theGlobal.GetSystemSet().strWorkshopName,
		theGlobal.m_ProductTool.GetCurrentProductName(), theGlobal.GetCurrentBatch());

	m_strImageInquire.Format("%s\\涂布CCD图像_%s\\%s\\", m_strMapFolderPath, theGlobal.GetSystemSet().strWorkshopName,
		theGlobal.m_ProductTool.GetCurrentProductName());

	m_strThumbImageSaveFolder.Format("%s\\涂布CCD图像_%s\\%s\\%s_Defect\\", m_strMapFolderPath, theGlobal.GetSystemSet().strWorkshopName,
		theGlobal.m_ProductTool.GetCurrentProductName(), theGlobal.GetCurrentBatch());
}
...
void CStorage::SaveData(C_MergeWork& tOutput)
{
	...
	//写首件文件
	WriteFirstArticleInspFile(tOutput);
}
...
void CStorage::SaveImage(C_MergeWork& tOutput)
{
	...
	HTuple hvQueueNum;
	GetMessageQueueParam(theGlobal.m_Storage.GetStorageQueue(0), "message_num", &hvQueueNum);
	if (hvQueueNum > 0)
	{
		CString strLog;
		strLog.Format("存储队列数量为%d,大于0,当前如果存OK图时将会跳过", hvQueueNum.I());
		...
		try
		{
			//if (tOutput.tMerge.vecSideResult[nBWorkSN].hv_Flag.I() != NG)
			{
				...
				if (bSave && hvQueueNum == 0)
				{
					...
					for (int i = 0; i < tOutput.vecB[nBWorkSN].vecA.size(); i++)
					{
						...
						strFile.Format("%s\\%s_Cam%d_%d_%s", m_strImageSaveFolder, GetSaveTimeS2(), tOutput.vecB[nBWorkSN].vecA[i].hvGrabberSN.I(), tOutput.vecB[nBWorkSN].vecA[i].hvImageSN.I(), strNGOK);
			...
			if (tOutput.vecB[nBWorkSN].tMeasure.tDefect.hv_Type.Length() > 0)
			{
				...
				for (int j = 0; j < tOutput.vecB[nBWorkSN].tMeasure.tDefect.hv_Type.Length(); j++)
				{
					...
					double hv_Area = tOutput.vecB[nBWorkSN].tMeasure.tDefect.tFeature.hvArea[j].D();
					strFile.Format("%s\\%s_%s_(Area%0.4fmm_X%0.4fmm_Y%-0.4fm)_Img%d_%s", m_strThumbImageSaveFolder, GetSaveTimeS2(),
						theGlobal.m_Counter.GetNGTypeName(tOutput.vecB[nBWorkSN].tMeasure.tDefect.hv_Type[j].I()), tOutput.vecB[nBWorkSN].tMeasure.tDefect.tFeature.hvArea[j].D(), dX, dY,
...
BOOL CStorage::CreateRecordFile(const CString& strFilePath)
{
	...
	strItem += ",图像号,模头位置(mm),左刀距纠偏差值(刀),右刀距纠偏差值(刀),左刀距闭环值(刀),右刀距闭环值(刀),左刀距位置(um),右刀距位置(um)\n";
	try
	{
		CStdioFile file;
		if (TRUE != file.Open(strFilePath, CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite))
...
BOOL CStorage::WriteGRRFile(C_MergeWork& tOutput)
{
	...
	for (int nBWorkSN = 0; nBWorkSN < tOutput.vecB.size(); nBWorkSN++)
	{
		...
		{
			if ((s_hvMisalign.Length() == 0 || m_bGrrNewStart == TRUE) && tOutput.vecB.size() > 1)
			{
				s_hvMisalign = tOutput.tMerge.hv_Misalignment;
			}
	...
	if (theGlobal.m_ProductTool.GetCurrentProductInfo().nCoatingType == TwoSide && theGlobal.m_Project.GetProjectConfig().nSideCount == 2)
	{
		...
		for (int i = 0; i < nNum; i++)
		{
			...
			{
				HTuple  hv_RandTuple;
				TupleGenConst(s_hvMisalign.Length(), 0, &hv_RandTuple);
				for (int j = 0; j < theGlobal.m_ProductTool.GetCurrentProductInfo().tParam.vecMisalignment.size(); j++)
				{
					...
					double dMis = 0;
					if (abs(hvMis[j].D()) < NO_DATA / 4)
						dMis = hvMis[j].D();

					CString strTmp;
					strTmp.Format("%0.4f,", dRandValue + dMis);
...
//		{
//			CString strTmp;
//			strTmp.Format("%s面左点检宽(mm),%s面左点检宽规格(mm),%s面左点检高(mm),%s面左点检高规格(mm),%s面右点检宽(mm),%s面右点检宽规格(mm),%s面右点检高(mm),%s面右点检规格(mm),%s面点检距离(mm),%s面点检距离规格(mm),%s面温度(℃),%s面温度限制(℃),",
//				nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反",
//				nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反",
//				nBWorkSN == 0 ? "正" : "反", nBWorkSN == 0 ? "正" : "反");
//			strName += strTmp;
//		}
//		std::ofstream outFile;
//		outFile.open(strFilePath, std::ios::out | std::ios::app);
...
//					else
//					{
//						strTmp.Format("%0.4f,%0.2f±%0.2f,", tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[j].D(), tPdInfo.tParam.vecChecking[j].dStandard2,
//							tPdInfo.tParam.vecChecking[j].dTolerance);
//					}
//				}
//				else
...
//				{
//					strTmp.Format("%0.1f,%0.1f,", (double)vTempList[nBWorkSN], theGlobal.m_Judge.m_tSpotInspParam.dMaxTemperature);
//					strValue += strTmp;
//				}
//				else
//				{
//					strTmp.Format(",%0.1f,", (double)vTempList[nBWorkSN], theGlobal.m_Judge.m_tSpotInspParam.dMaxTemperature);
//					strValue += strTmp;
//				}	
//			}
//			else
...
BOOL CStorage::WriteSpotInspectionFile(C_MergeWork& tOutput)
{
	//写入点检数据
	tagPDInfo tPdInfo = theGlobal.m_ProductTool.GetCurrentProductInfo();
	if (!tPdInfo.tParam.bUseChecking || theGlobal.m_tSystemSet.nActionMode != RunMode || !theGlobal.m_Judge.m_tSpotInspParam.bStart) return TRUE;
	...
	if (tOutput.vecB.size() > 1)
	{
		...
		HTuple hvInc;
		TupleFind(hvVal, NO_DATA, &hvInc);
		if (hvInc.Length() >= 14) return FALSE;
	}
	else
	{
		HTuple hvVal;
		...
		HTuple hvInc;
		TupleFind(hvVal, NO_DATA, &hvInc);
		if (hvInc.Length() >= 7) return FALSE;
	}


	CString strFilePath;
	strFilePath.Format("%s\\点检数据\\%04d%02d\\", m_strMapFolderPath, m_tSaveTime.wYear, m_tSaveTime.wMonth);
	if (!PathFileExists(strFilePath)) CreateMultilevelPath(strFilePath);
	...
	for (int nBWorkSN = 0; nBWorkSN < tOutput.vecB.size(); nBWorkSN++)
	{
		...
		strValue.Format("%s,", GetSaveTimeS3());
		strFilePath.Format("%s\\点检数据\\%04d%02d\\%s面原始数据.csv", m_strMapFolderPath,
			m_tSaveTime.wYear, m_tSaveTime.wMonth, nBWorkSN == 0 ? "正" : "反");
		HalconCpp::FileExists((HTuple)strFilePath, &hvExist);
		if (hvExist == 0)
		{
			strName = "点检时间,温度下限,温度,直径11_标准,直径11_公差(±),直径11_实测,直径12_标准,直径12_公差(±),直径12_实测,圆心距1_标准,圆心距1_公差(±),圆心距1_实测,"
				"直径21_标准,直径21_公差(±),直径21_实测,直径22_标准,直径22_公差(±),直径22_实测,圆心距2_标准,圆心距2_公差(±),圆心距2_实测,"
				"圆心距3_标准,圆心距3_公差(±),圆心距3_实测,";
		...
		if (tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width.Length() != 7) continue;
		...
		if (theGlobal.m_Judge.m_tSpotInspParam.bUseTemp && bConnect)
		{
			if (vTempList.size() > nBWorkSN)
			{
				strTmp.Format("%0.1f,%0.1f,", theGlobal.m_Judge.m_tSpotInspParam.dMaxTemperature, (double)vTempList[nBWorkSN]);
				strValue += strTmp;
			}
			else
			{
		...
		//直径11 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[0].dStandard : tPdInfo.tParam.vecChecking[0].dStandard2,
			tPdInfo.tParam.vecChecking[0].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[0].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[0].D());
		strValue += strTmp;
		//直径12 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[1].dStandard : tPdInfo.tParam.vecChecking[1].dStandard2,
			tPdInfo.tParam.vecChecking[1].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[1].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[1].D());
		strValue += strTmp;
		//圆心距1 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[2].dStandard : tPdInfo.tParam.vecChecking[2].dStandard2,
			tPdInfo.tParam.vecChecking[2].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[2].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[2].D());
		strValue += strTmp;
		//直径21 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[3].dStandard : tPdInfo.tParam.vecChecking[3].dStandard2,
			tPdInfo.tParam.vecChecking[3].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[3].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[3].D());
		strValue += strTmp;
		//直径22 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[4].dStandard : tPdInfo.tParam.vecChecking[4].dStandard2,
			tPdInfo.tParam.vecChecking[4].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[4].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[4].D());
		strValue += strTmp;
		//圆心距2 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[5].dStandard : tPdInfo.tParam.vecChecking[5].dStandard2,
			tPdInfo.tParam.vecChecking[5].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[5].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[5].D());
		strValue += strTmp;
		//圆心距3 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[6].dStandard : tPdInfo.tParam.vecChecking[6].dStandard2,
			tPdInfo.tParam.vecChecking[6].dTolerance, tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[6].D() <= 0 ? -1 : tOutput.vecB[nBWorkSN].tMeasure.tSpotInspection.hv_Width[6].D());
		strValue += strTmp;
...
BOOL CStorage::WriteSpotInspectionReport(const CJudge::tagSpotInspCount& tCount)
{
	...
	{
		CString strName = "生产工序,机台编号,点检时间,判定结果,温度,";
		strFilePath.Format("%s\\点检数据\\%04d%02d\\%s面判定表.csv", m_strMapFolderPath,
			m_tSaveTime.wYear, m_tSaveTime.wMonth, nBWorkSN == 0 ? "正" : "反");
		HalconCpp::FileExists((HTuple)strFilePath, &hvExist);
		if (hvExist == 0)
		{
			strName = "生产工序,机台编号,点检时间,判定结果,温度下限,温度,直径11_标准,直径11_公差(±),直径11_实测,直径12_标准,直径12_公差(±),直径12_实测,圆心距1_标准,圆心距1_公差(±),圆心距1_实测,"
				"直径21_标准,直径21_公差(±),直径21_实测,直径22_标准,直径22_公差(±),直径22_实测,圆心距2_标准,圆心距2_公差(±),圆心距2_实测,"
				"圆心距3_标准,圆心距3_公差(±),圆心距3_实测,";
		...
		if (tCount.hvDataMedian[nBWorkSN].Length() != 7) continue;

		CString strTmp, strTime, strValue;
		strTime.Format("%04d/%02d/%02d %02d:%02d:%02d", tTime.wYear, tTime.wMonth, tTime.wDay,
			tTime.wHour, tTime.wMinute, tTime.wSecond);
		//生产工序,机台编号,点检时间,判定结果,
		strValue.Format("涂布,%s,%s,%s,", theGlobal.m_tSystemSet.strDeviceID, strTime, tCount.hvFlagSingle[nBWorkSN] == NG ? "NG" : "OK");
		//温度
		if (theGlobal.m_Judge.m_tSpotInspParam.bUseTemp)
		{
			if (tCount.hvTempMedian[nBWorkSN].Length() > 0 && tCount.hvTempMedian[nBWorkSN].D() > 0)
			{
				strTmp.Format("%0.1f,%0.1f,", theGlobal.m_Judge.m_tSpotInspParam.dMaxTemperature, (double)tCount.hvTempMedian[nBWorkSN].D());
				strValue += strTmp;
			}
			else
			{
		...
		//直径11 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[0].dStandard : tPdInfo.tParam.vecChecking[0].dStandard2,
			tPdInfo.tParam.vecChecking[0].dTolerance, tCount.hvDataMedian[nBWorkSN][0].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][0].D());
		strValue += strTmp;
		//直径12 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[1].dStandard : tPdInfo.tParam.vecChecking[1].dStandard2,
			tPdInfo.tParam.vecChecking[1].dTolerance, tCount.hvDataMedian[nBWorkSN][1].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][1].D() <= 0);
		strValue += strTmp;
		//圆心距1 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[2].dStandard : tPdInfo.tParam.vecChecking[2].dStandard2,
			tPdInfo.tParam.vecChecking[2].dTolerance, tCount.hvDataMedian[nBWorkSN][2].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][2].D() <= 0);
		strValue += strTmp;
		//直径21 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[3].dStandard : tPdInfo.tParam.vecChecking[3].dStandard2,
			tPdInfo.tParam.vecChecking[3].dTolerance, tCount.hvDataMedian[nBWorkSN][3].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][3].D() <= 0);
		strValue += strTmp;
		//直径22 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[4].dStandard : tPdInfo.tParam.vecChecking[4].dStandard2,
			tPdInfo.tParam.vecChecking[4].dTolerance, tCount.hvDataMedian[nBWorkSN][4].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][4].D() <= 0);
		strValue += strTmp;
		//圆心距2 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[5].dStandard : tPdInfo.tParam.vecChecking[5].dStandard2,
			tPdInfo.tParam.vecChecking[5].dTolerance, tCount.hvDataMedian[nBWorkSN][5].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][5].D() <= 0);
		strValue += strTmp;
		//圆心距3 标准，公差，实测
		strTmp.Format("%0.3f,%0.3f,%0.3f,", nBWorkSN == 0 ? tPdInfo.tParam.vecChecking[6].dStandard : tPdInfo.tParam.vecChecking[6].dStandard2,
			tPdInfo.tParam.vecChecking[6].dTolerance, tCount.hvDataMedian[nBWorkSN][6].D() <= 0 ? -1 : tCount.hvDataMedian[nBWorkSN][6].D() <= 0);
		strValue += strTmp;
...
BOOL CStorage::CreateFirstArticleInspReport(const CString& strFilePath)
{
	//首件数据列表
	tagPDInfo tPDInfo = theGlobal.m_ProductTool.GetCurrentProductInfo();
	CString strItem, strTmp, strSide;
...
BOOL CStorage::WriteFirstArticleInspReport(const CJudge::tagFirstArticleInspCount& tCount)
{
	//判断有无涂膜不计入统计
	tagPDInfo tPdInfo = theGlobal.m_ProductTool.GetCurrentProductInfo();
	CString strFilePath;
	...
	if (!PathFileExists(strFilePath)) CreateMultilevelPath(strFilePath);
	strFilePath.Format("%s\\首件数据\\首件判定表_%04d%02d%02d_%s_%s.csv", m_strMapFolderPath, m_tSaveTime.wYear, m_tSaveTime.wMonth, m_tSaveTime.wDay
		, theGlobal.GetCurrentBatch(), tPdInfo.strPDName);

	if (!PathFileExists(strFilePath))
	{
		if (FALSE == CreateFirstArticleInspReport(strFilePath))
		{
			CString strException;
			strException.Format("Storage_WriteFirstArticleFile: 首件报表文件被打开,创建文件失败");
			theGlobal.m_Log.Exception(strException);
			return FALSE;
		}
	}
	...
	try
	{
		for (int n = 0; n < tCount.hvecDataFilterList[0].Length(); n++)
		{
			//配方名，膜卷号，时间
			...
			HTuple hvSize;
			//正面数据
			hvSize = tCount.hvecDataFilterList[0][n].T();
			for (int i = 0; i < tPdInfo.vecNameList.size(); i++)
			{
			...
			if (tPdInfo.nCoatingType == TwoSide)
			{
				hvSize = tCount.hvecDataFilterList[1][n].T();
				//反面数据
				for (int i = 0; i < tPdInfo.vecNameList.size(); i++)
				{
					if (i > hvSize.Length() - 1)
				...
				//错位
				hvSize = tCount.hvecMisaglinDataFilterList[n].T();
				for (int i = 0; i < tPdInfo.tParam.vecMisalignment.size(); i++)
				{
			...
			//结果
			if (n == tCount.hvecDataFilterList[0].Length() - 1)
			{
			...
			{
				CString strException;
				strException.Format("Storage_WriteFirstArticleInspReport: 点检报表文件被打开,创建文件失败");
				theGlobal.m_Log.Exception(strException);
				return FALSE;
			}
			file.SeekToEnd();
	...
	{
		CString strException;
		strException.Format("Storage_WriteFirstArticleInspReport: Error:%s  [ %s() ]", e.ErrorMessage().Text()
			, e.ProcName().Text());
		theGlobal.m_Log.Exception(strException);
	}
...
void CStorage::WriteRecordFile(const CString& strFilePath, C_MergeWork& tOutput)
{
	...
	{
		if (theGlobal.m_tSystemSet.eDevType == CGlobal::DT_SingleLayer)
		{
			if (1 != tOutput.vecB[0].vecA[0].hvRunDirection.I()
				|| 0 == tOutput.vecB[0].vecA[0].hvRunDirection[1].I()
				|| 0 == tOutput.vecB[0].vecA[0].hvRunDirection[2].I()) return;
		}
		else if (theGlobal.m_tSystemSet.eDevType == CGlobal::DT_DoubleLayer)
		{
			HTuple hvHasCoat = FALSE;
			if (tOutput.vecB[0].tMeasure.hv_HasCoat.I() == TRUE) hvHasCoat = TRUE;
			if (tOutput.vecB.size() > 1 && tOutput.vecB[1].tMeasure.hv_HasCoat.I() == TRUE) hvHasCoat = TRUE;
			if (hvHasCoat.I() == FALSE) return;
		}
		//测量数据滤波，与标准值差值超过3mm就不计入统计
		else if (!theGlobal.m_Counter.GetCountDataFilteringEnble(tOutput))
		{
			return;
		}
	}
	...
	try
	{
		//a：第1列必须是膜卷号，且时间和ID为必须项，1~3列为膜卷号、时间、ID	、走带速度												
		//b：第4~第24列为尺寸数据，如下图例为1出6，记录13个尺寸数据；原始数据按照1出10，记录21个尺寸数据，预留位置，实际情况无值，返回NA；比如，1出6时，a13~a21返回值均为NA；B面类推，在第25列~45列
		//c：第46~52列记录涂布外观NG重量，此处外观不良返回“1”，外观OK返回“0”
		//d：第53列记录浆料信息，如果更换浆料，此列显示新浆料号
		//d：第54~59列记录基本信息，但为必须项
		...
		strItem.Format("%s,%s,%s,%0.2f", theGlobal.GetCurrentBatch(), GetSaveTimeS(), "ID",theGlobal.m_Speed.GetSpeed());
		...
		for (int i = 0; i < theGlobal.m_CommunicationTool.m_nTempertatureNums; i++)
		{
			if (i < vTempList.size())
			{
				...
				strTmp.Format(",%s", (!bConnect) ? "" : strTemp);
				strItem += strTmp;
			}
			else
			{
		...
		strItem += strTemp;
		HTuple hvKnifeDiffValue;
		bRet = theGlobal.m_CommunicationTool.DoSomething(CCommunicationTool::PLC, "GetPLCKnifeDiffValue", NULL, &hvKnifeDiffValue);
		//左刀距纠偏差值
		if (bRet)
		{
			strTemp.Format(",%0.4f", hvKnifeDiffValue[0].D());
			strItem += strTemp;
		}
		else
		{
		...
		if (bRet)
		{
			strTemp.Format(",%0.4f", hvKnifeDiffValue[1].D());
			strItem += strTemp;
		}
		else
		{
		...
		HTuple hvKnifeCorrectValue;
		bRet = theGlobal.m_CommunicationTool.DoSomething(CCommunicationTool::PLC, "GetPLCKnifeCorrectValue", NULL, &hvKnifeCorrectValue);
		//左刀距闭环值
		if (bRet)
		{
			strTemp.Format(",%0.4f", hvKnifeCorrectValue[0].D());
			strItem += strTemp;
		}
		else
		{
		...
		if (bRet)
		{
			strTemp.Format(",%0.4f", hvKnifeCorrectValue[1].D());
			strItem += strTemp;
		}
		else
		{
		...
		//左刀距位置
		strTemp.Format(",%0.6f", theGlobal.m_CommunicationTool.m_dPlcLeftKnifePosition);
		strItem += strTemp;
		//右刀距位置
		strTemp.Format(",%0.6f", theGlobal.m_CommunicationTool.m_dPlcRightKnifePosition);
		strItem += strTemp;
		strItem += "\n";
...
void CStorage::WriteRecordFileAll(const CString& strFilePath, C_MergeWork& tOutput)
{
	...
	try
	{
		//a：第1列必须是膜卷号，且时间和ID为必须项，1~4列为膜卷号、时间、ID	、走带速度											
		//b：第4~第24列为尺寸数据，如下图例为1出6，记录13个尺寸数据；原始数据按照1出10，记录21个尺寸数据，预留位置，实际情况无值，返回NA；比如，1出6时，a13~a21返回值均为NA；B面类推，在第25列~45列
		//c：第46~52列记录涂布外观NG重量，此处外观不良返回“1”，外观OK返回“0”
		//d：第53列记录浆料信息，如果更换浆料，此列显示新浆料号
		//d：第54~59列记录基本信息，但为必须项
	...
	{
		CString strException;
		strException.Format("Storage_WriteRecordFileAll:ImageSN:%d Error:%s  [ %s() ]", tOutput.vecB[0].vecA[0].hvImageSN.I(), e.ErrorMessage().Text()
			, e.ProcName().Text());
		theGlobal.m_Log.Exception(strException);
	}
}
...
BOOL CStorage::IsDiskRemainEnough()
{
	...
	if (TRUE == bFull)
	{
		...
		{
			CString strException;
			strException.Format("[%s]磁盘空间小于%dGB，图像和数据将停止保存)",
				m_strMapFolderPath, m_nMinRemainStorage);
...
void CStorage::DeleteOldCaches()
{
	...
	//if (s_bDelOldCaches)
	{
		...
		while (bRet)
		{
			...
			if (strDatePath.Find("图像") == -1)continue;
			...
			while (bRet2)
			{
				...
				while (bRet3)
				{
					...
					CString strDateName = datePathFinder2.GetFileTitle();//获取文件夹标题
					CTime t2;
					datePathFinder2.GetCreationTime(t2);
					...
					try
					{
						...
						CTime t3(t2.GetYear(), t2.GetMonth(), t2.GetDay(), 0, 0, 0);
						CTimeSpan tSpan = t1 - t3;
						int day = tSpan.GetDays();
						//.....判断遍历到的文件夹否是过期....
...
//写首件数据
BOOL CStorage::WriteFirstArticleInspFile(C_MergeWork& tOutput)
{
	if (!theGlobal.m_Judge.m_tFirstArticleInspCount.bStart) return TRUE;
