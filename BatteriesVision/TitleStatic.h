/*===========================================================================
====    Project : CTitleStatic (www.codeproject.com)                         ====
====    File name           :  CTitleStatic.h                                ====
...
public:
	...
	void	SetBkColorNormal(COLORREF rgbTitle=RGB(48,48,48), COLORREF rgbText=RGB(48,48,48)){m_rgbTitleBkgnd=rgbTitle;m_rgbBkgnd = rgbText;m_fillmode = Normal;}
	void	SetTransparent(BOOL bTranspMode);
	void	SetTextColor(COLORREF col);
	void	SetTextColorAll(COLORREF colTitle,COLORREF colText);
	void	SetFont(const CString& strFont, int nPointSize,int nWeight, BOOL bRedraw = TRUE);
	void    SetFontAll(const CString& strTitleFont, int nTitlePointSize,int nTitleWeight,const CString& strTextFont, int nTextPointSize,int nTextWeight, BOOL bRedraw = TRUE);
	void	SetFont(LOGFONT *pLogFont, BOOL bRedraw = TRUE);
	void	SetFont(CFont *pFont, BOOL bRedraw = TRUE);
	void	SetFont3D(BOOL bFont3D, Type3D type);
	void	SetTitleSize(int nSize){m_nTitleSize = nSize;}
	void	SetStaticHorizontal(int bHorizontal){m_bHorizontal = bHorizontal;}
	...
	void	SetTitleText(LPCTSTR strText);
