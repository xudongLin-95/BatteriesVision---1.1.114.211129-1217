...
void CPageProductSet::InitUI()
{
	...
	m_Grid_ProductList.Create(WS_CHILD, rcGridBase, this, 1000);//WS_CHILD表示生成的窗口是母窗口的子窗口，1000是标识符
	...
	RefeshProductList();
	m_Grid_ProductList.SelectRow(m_nCurrentProductSel);//选择行
...
void CPageProductSet::RefeshProductList()
{
	...
	{
		CBCGPGridRow* pRow = m_Grid_ProductList.CreateRow(m_Grid_ProductList.GetColumnCount());//创建行，根据列数量
	...
	m_Grid_ProductList.AdjustLayout();//调整布局
}
...
#pragma endregion
	...
	pRow = m_Grid_ProductParam.GetRow(nCount++);
	tPdInfo.tParam.vecCoatWidth[0].dFAITolerance = pRow->GetItem(2 + 0)->GetValue();
	pRow = m_Grid_ProductParam.GetRow(nCount++);
	for (int i = 0; i < tPdInfo.tParam.vecCoatWidth.size(); i++)
	{
	...
	for (int i = 0; i < tPdInfo.tParam.vecFoilWidth.size(); i++)
	{
		tPdInfo.tParam.vecFoilWidth[i].dFAITolerance = pRow->GetItem(2 + i)->GetValue();
	}
	pRow = m_Grid_ProductParam.GetRow(nCount++);
	for (int i = 0; i < tPdInfo.tParam.vecFoilWidth.size(); i++)
	{
	...
	if (tPdInfo.bExistAT11)
	{
		...
		//for (int i = 0; i < tPdInfo.tParam.vecAT11Width.size(); i++)
		{
			tPdInfo.tParam.vecAT11Width[0].dFAITolerance = pRow->GetItem(2 + 0)->GetValue();
		}
		pRow = m_Grid_ProductParam.GetRow(nCount++);
		for (int i = 0; i < tPdInfo.tParam.vecAT11Width.size(); i++)
		{
	...
	else if (tPdInfo.bExistBaseCoat)
	{
		...
		//for (int i = 0; i < tPdInfo.tParam.vecBaseCoatWidth.size(); i++)
		{
			tPdInfo.tParam.vecBaseCoatWidth[0].dFAITolerance = pRow->GetItem(2 + 0)->GetValue();
		}
		pRow = m_Grid_ProductParam.GetRow(nCount++);
		for (int i = 0; i < tPdInfo.tParam.vecBaseCoatWidth.size(); i++)
		{
	...
	if (tPdInfo.nCoatingType == TwoSide && theGlobal.m_Project.GetProjectConfig().nSideCount > 1)
	{
		...
		//for (int i = 0; i < tPdInfo.tParam.vecMisalignment.size(); i++)
		{
			tPdInfo.tParam.vecMisalignment[0].dFAITolerance = pRow->GetItem(2 + 0)->GetValue();
		}
		pRow = m_Grid_ProductParam.GetRow(nCount++);
		for (int i = 0; i < tPdInfo.tParam.vecMisalignment.size(); i++)
		{
...
void CPageProductSet::RefreshProductParam(tagPDInfo& tPdInfo)
{
	...
	m_Grid_ProductParam.AddRow(pRow, FALSE);
	pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	pRow->GetItem(0)->SetValue(_variant_t("涂膜首件公差(mm)"), FALSE);
	pRow->GetItem(1)->AllowEdit(FALSE);
	//for (int i = 0; i < tPdInfo.tParam.vecCoatWidth.size(); i++)
	{
		pRow->GetItem(2 + 0)->SetValue(_variant_t(tPdInfo.tParam.vecCoatWidth[0].dFAITolerance), FALSE);
	}
	m_Grid_ProductParam.AddRow(pRow, FALSE);
	pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	...
	m_Grid_ProductParam.AddRow(pRow, FALSE);
	pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	pRow->GetItem(0)->SetValue(_variant_t("极耳首件公差(mm)"), FALSE);
	pRow->GetItem(1)->AllowEdit(FALSE);
	for (int i = 0; i < tPdInfo.tParam.vecFoilWidth.size(); i++)
	{
		pRow->GetItem(2 + i)->SetValue(_variant_t(tPdInfo.tParam.vecFoilWidth[i].dFAITolerance), FALSE);
	}
	m_Grid_ProductParam.AddRow(pRow, FALSE);
	pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	...
	if (tPdInfo.bExistAT11)
	{
		...
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
		pRow->GetItem(0)->SetValue(_variant_t("AT11首件公差(mm)"), FALSE);
		pRow->GetItem(1)->AllowEdit(FALSE);
		//for (int i = 0; i < tPdInfo.tParam.vecAT11Width.size(); i++)
		{
			pRow->GetItem(2 + 0)->SetValue(_variant_t(tPdInfo.tParam.vecAT11Width[0].dFAITolerance), FALSE);
		}
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	...
	else if (tPdInfo.bExistBaseCoat)
	{
		...
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
		pRow->GetItem(0)->SetValue(_variant_t("凹版首件公差(mm)"), FALSE);
		pRow->GetItem(1)->AllowEdit(FALSE);
		//for (int i = 0; i < tPdInfo.tParam.vecBaseCoatWidth.size(); i++)
		{
			pRow->GetItem(2 + 0)->SetValue(_variant_t(tPdInfo.tParam.vecBaseCoatWidth[0].dFAITolerance), FALSE);
		}
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
	...
	if (tPdInfo.nCoatingType == TwoSide && theGlobal.m_Project.GetProjectConfig().nSideCount > 1)
	{
		...
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
		pRow->GetItem(0)->SetValue(_variant_t("错位首件公差(mm)"), FALSE);
		pRow->GetItem(1)->AllowEdit(FALSE);
		//for (int i = 0; i < tPdInfo.tParam.vecMisalignment.size(); i++)
		{
			pRow->GetItem(2 + 0)->SetValue(_variant_t(tPdInfo.tParam.vecMisalignment[0].dFAITolerance), FALSE);
		}
		m_Grid_ProductParam.AddRow(pRow, FALSE);
		pRow = m_Grid_ProductParam.CreateRow(m_Grid_ProductParam.GetColumnCount());
