...
extern CEvent	g_evtComplete;		//检测是否完成
extern CEvent	g_evtStart;			//检测是否开始
...
public:
	...
	CDefect& operator= (const CDefect& in);
private:
	//动态阈值粗检缺陷
	BOOL GetDynRegion();
...
public:
	...
	HDevThread*		m_pThreadHdl;			//线程句柄

private:
	BOOL    m_bDetectEnd;		//检测是否结束
	int		m_nMaxDetectTime;	//检测最大耗时ms
