	...
	try
	{
		...
		for (int i = 0; i < m_tParam.hoSearchReigon.CountObj(); i++)
		{
			...
			if (m_tParam.eColorType == IC_Light)
			{
				...
				{
					HObject hoRgnTmp;
					SelectObj(hoRegion, &hoRgnTmp, i + 1);
					SelectGray(hoRgnTmp, ho_ImageCrop, &hoRgnTmp, "mean", "and", min(m_tParam.nThres, 255), 255);
					if (hoRgnTmp.CountObj() > 0)
					{
						ConcatObj(hoRegionOpening, ho_SelectedXLD.SelectObj(i + 1), &hoRegionOpening);
						break;
					}
				}
			}
			else
			{
				...
				for (int i = 0; i < hoRegion.CountObj(); i++)
				{
					HObject hoRgnTmp;
					SelectObj(hoRegion, &hoRgnTmp, i + 1);
					SelectGray(hoRgnTmp, ho_ImageCrop, &hoRgnTmp, "mean", "and", 0, min(m_tParam.nThres, 255));
					if (hoRgnTmp.CountObj() > 0)
					{
						ConcatObj(hoRegionOpening, ho_SelectedXLD.SelectObj(i + 1), &hoRegionOpening);
						break;
			...
			if (hv_Row2 > hv_Height - 50 || hv_Row < 50)
			{
				tOutTmp.ptCenter.hv_Column.Append(NO_DATA);
				tOutTmp.ptCenter.hv_Row.Append(NO_DATA);
				tOutTmp.ptLeftTop.hv_Column.Append(NO_DATA);
			...
			tOutTmp.ptCenter.hv_Column.Append(hvCenterCol + hv_Column1);
			tOutTmp.ptLeftTop.hv_Row.Append(hv_Row);
			tOutTmp.ptLeftTop.hv_Column.Append(hv_Column + hv_Column1);
			tOutTmp.ptRightBottom.hv_Row.Append(hv_Row2);
			tOutTmp.ptRightBottom.hv_Column.Append(hv_Column2 + hv_Column1);
