		...
		//for (int i = 0; i < 1; i++)
		{
			...
			try
			{
				...
				if (theGlobal.m_Upload.m_bConnectClient == TRUE)
				{
					...
					if ((hvTimeTmp.D() - hvTimeStampe.D() > 60) && bConnectAlarm)
					{
						...
						CountSeconds(&hvTimeStampe);
						CString strMessage;
						strMessage.Format("1分钟未收到工位%d的信息，已断开网络", theGlobal.m_tSystemSet.eSN + 1);
			...
			if ("tuple" == strRecvData)
			{
				...
				try
				{
					...
					{
						CString strlog;
						strlog.Format("接收工位%d发送心跳成功", theGlobal.m_tSystemSet.eSN + 1);
						//theGlobal.m_Log.Communication(strlog);
						//反馈心跳
					...
					static HTuple s_hvCount = HTuple().TupleGenConst(1, 0);
					if (theGlobal.m_Upload.m_bSendCheckProductInfo == FALSE)
					{
						s_hvCount += 1;
						if (s_hvCount > 60000)//2分钟没有收到配方校验反馈，不再接收
						{
							s_hvCount = 0;
							theGlobal.m_Upload.m_bSendCheckProductInfo = TRUE;
							CString strInfo;
...
void CUpload::WriteBiasFile(HTuple hvBiasValue, int nImageSN)
{
	...
	try
	{
		...
		if (!PathFileExists(strCsvFile))
		{
			...
			{
				strTmp.Format("%s,", tPdInfo.vecNameList);
				strName += strTmp;
			}
			strName += "\n";
			CStdioFile fl;
		...
		{
			strTmp.Format("%0.4f,", hvBiasValue.D());
			strValue += strTmp;
		}
		strValue += "\n";
