...
public:
	...
	//写串口信息
	bool Write(const unsigned char *puchMsg);
	//读串口信息
	bool Read(unsigned char *puchMsg, DWORD dwReceiveLength);
	//读取单个温度
	bool GetTemperature(unsigned char rtu, float &temp);
	//读取多个温度
	bool GetTemperatureList(std::vector<float> &tempList);
	//
