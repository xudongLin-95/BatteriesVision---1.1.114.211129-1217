...
void CMeasure::SpotInspect()
{
	...
	try
	{
		...
		else if (m_SpotInspection.GetType() == CSpotInspection::IT_Circle && tOut.ptCenter.hv_Row.Length() >= 4)
		{
			//m_ptOut->tSpotInspection.hv_Width.Clear();
			//2021.6.9修改为通常一个检测面两个点检目标
			DoubleList OffsetList;
			...
			//左侧圆1的直径
			int nSN = 0;
			if (tOut.ptLeftTop.hv_Column[nSN] !=NO_DATA && tOut.ptRightBottom.hv_Column[nSN]!=NO_DATA)
			{
				tPPIn.hvCol1 = tOut.ptLeftTop.hv_Column[nSN];
				tPPIn.hvCol2 = tOut.ptRightBottom.hv_Column[nSN];
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[nSN].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[0].dStandard, m_tInit.tProductInfo.tParam.vecChecking[0].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[0] = hvSizeTmp;
			}
			...
			if (tOut.ptLeftTop.hv_Column[nSN] != NO_DATA && tOut.ptRightBottom.hv_Column[nSN] != NO_DATA)
			{
				...
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[0].dStandard, m_tInit.tProductInfo.tParam.vecChecking[0].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[1] = hvSizeTmp;
			}
			

			//左侧圆1和圆2的圆心间距
			if (tOut.ptCenter.hv_Column[0] != NO_DATA && tOut.ptCenter.hv_Column[1] != NO_DATA)
			{
				tPPIn.hvCol1 = tOut.ptCenter.hv_Column[0];
				tPPIn.hvCol2 = tOut.ptCenter.hv_Column[1];
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[2].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[1].dStandard, m_tInit.tProductInfo.tParam.vecChecking[1].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[2] = hvSizeTmp;
			}
			...
			if (tOut.ptLeftTop.hv_Column[nSN] != NO_DATA && tOut.ptRightBottom.hv_Column[nSN] != NO_DATA)
			{
				...
				tPPIn.hvCol2 = tOut.ptRightBottom.hv_Column[nSN];
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[3].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[0].dStandard, m_tInit.tProductInfo.tParam.vecChecking[0].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[3] = hvSizeTmp;
			}
			...
			if (tOut.ptLeftTop.hv_Column[nSN] != NO_DATA && tOut.ptRightBottom.hv_Column[nSN] != NO_DATA)
			{
				...
				tPPIn.hvCol2 = tOut.ptRightBottom.hv_Column[nSN];
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[4].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[0].dStandard, m_tInit.tProductInfo.tParam.vecChecking[0].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[4] = hvSizeTmp;
			}
			

			//右侧圆3和圆4的圆心间距
			if (tOut.ptCenter.hv_Column[2] != NO_DATA && tOut.ptCenter.hv_Column[3] != NO_DATA)
			{
				tPPIn.hvCol1 = tOut.ptCenter.hv_Column[2];
				tPPIn.hvCol2 = tOut.ptCenter.hv_Column[3];
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[5].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[1].dStandard, m_tInit.tProductInfo.tParam.vecChecking[1].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[5] = hvSizeTmp;
			}

			//左右圆心的圆间距，左右圆心的中心的间距
			if (tOut.ptCenter.hv_Column[0]!=NO_DATA && tOut.ptCenter.hv_Column[1] != NO_DATA
				&& tOut.ptCenter.hv_Column[2] != NO_DATA && tOut.ptCenter.hv_Column[3] != NO_DATA)
			{
				tPPIn.hvCol1 = (tOut.ptCenter.hv_Column[0].D() + tOut.ptCenter.hv_Column[1].D()) / 2;
				tPPIn.hvCol2 = (tOut.ptCenter.hv_Column[2].D() + tOut.ptCenter.hv_Column[3].D()) / 2;
				m_Calibration.CalDistancePP(tPPIn, tPPOut);
				OffsetList = m_tInit.tProductInfo.tParam.vecChecking[6].OffsetList;
				hvSizeTmp = tPPOut.Dist.D() + OffsetList[m_tInit.nSN];
				//如果需要上手段，就把_CorrectValue打开
				//_CorrectValue(hvSizeTmp, m_tInit.tProductInfo.tParam.vecChecking[1].dStandard, m_tInit.tProductInfo.tParam.vecChecking[1].dTolerance, &hvSizeTmp);
				m_ptOut->tSpotInspection.hv_Width[6] = hvSizeTmp;
			}
...
void CMeasure::InitOutputValue()
{
	...
	TupleGenConst(m_tInit.tProductInfo.nMeasureLineNum - 1, NO_DATA, &m_ptOut->tSize.hv_WidthOrigin);
	...
	TupleGenConst(7, NO_DATA, &m_ptOut->tSpotInspection.hv_Width);
	GenEmptyObj(&m_ptOut->tSpotInspection.ho_Region);
