...
void CDlgParam::RefreshTestList()
{
	...
	m_tOutput.tSize.hv_WidthOrigin;
	...
	{
		for (int i = 0; i < m_tOutput.tSpotInspection.hv_Width.Length(); i = i + 3)
		{
			if (i + 1 >= m_tOutput.tSpotInspection.hv_Width.Length())
			{
				...
				{
					pRow = m_listInfo.CreateRow(m_listInfo.GetColumnCount());
					pRow->GetItem(0)->SetValue(_variant_t("点检两侧间距(mm)"), FALSE);
			...
			pRow = m_listInfo.CreateRow(m_listInfo.GetColumnCount());
			pRow->GetItem(0)->SetValue(_variant_t("点检[宽1,宽2,间距](mm)"), FALSE);
			if (m_tOutput.tSpotInspection.hv_Width[i] == NO_DATA)
				strVal = "-";
			else
				strVal.Format("%0.4f", m_tOutput.tSpotInspection.hv_Width[i].D());
			...
			pRow->GetItem(2)->SetValue(_variant_t(strVal), FALSE);
			if (m_tOutput.tSpotInspection.hv_Width[i + 2] == NO_DATA)
				strVal = "-";
			else
				strVal.Format("%0.4f", m_tOutput.tSpotInspection.hv_Width[i + 2].D());
			pRow->GetItem(3)->SetValue(_variant_t(strVal), FALSE);
			m_listInfo.AddRow(pRow, FALSE);
		}
	}
