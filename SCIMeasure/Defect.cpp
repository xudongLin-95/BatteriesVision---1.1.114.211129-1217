...
BOOL CDefect::ReadParam()
{
	...
	m_tParameters.tPretreatParam.nMaxDefectNum = GetPrivateProfileInt("tPretreatParam", "nMaxDefectNum", 32, m_strParamFile);
...
BOOL CDefect::WriteParam()
{
	...
	WritePrivateProfileInt("tPretreatParam", "nMaxDefectNum", m_tParameters.tPretreatParam.nMaxDefectNum, m_strParamFile);
...
BOOL CDefect::AddError(HObject ho_ErrorObject, e_ErrorType eErrorType, eErrorRegionType eType)
{
	...
	for (int i = 0; i < tRegion.ho_Region.CountObj(); i++)
	{
		...
		//暂时不限制最大输出缺陷数量 2021.4.21 限制最大数量最大输出32个
		if (m_ptOut->hv_Type.Length() >= m_tParameters.tPretreatParam.nMaxDefectNum) break;
...
void CDefect::LeakFoilDetect()
{
	try
	{
		...
		if ((m_tInit.tProductInfo.bExistAT11 || m_tInit.tProductInfo.bExistBaseCoat) && m_hoAT11BaseCoatRegion.CountObj() > 0)
		{
			...
			if (ho_ErrorRegion.CountObj() > 0)
			{
				if(m_tInit.tProductInfo.bExistAT11)
					...
					AddError(ho_ErrorRegion, NG_ExposeFoil, RT_BaseCoat);
				ho_ErrorRegion.GenEmptyObj();
			}
...
BOOL CDefect::GetDynRegion(HObject& ho_Image)
{
	try
	{
		//涂布数量大于2，单个涂布等分成5份
		...
		double dZoomFactor = max(0.1, min(m_tParameters.tPretreatParam.dZoomFactor, 1));
		//两边各腐蚀5个像素，防止没涂好误检
		ErosionRectangle1(m_hoZoomCoatMid, &hoCoat, 2, 1);
		//缩放图像和涂布区域，为了减低检测耗时
...
void CDefect::BlackDotDetect()
{
	...
	try
	{
		...
		hv_dMinArea = m_tParameters.tDarkDotParam.dMinArea / m_dColResolution / m_dRowResolution;
		hv_dMaxArea = m_tParameters.tDarkDotParam.dMaxArea / m_dColResolution / m_dRowResolution;
		...
		//涂膜边缘黑点
		HObject hoOpeningRegion, hoEdgeRegion, hoReduceDomain, hoRegion, hoSelectRegions;
		ErosionRectangle1(m_hoCoatMid, &hoOpeningRegion, 20 / m_dColResolution, 1);
		Difference(m_hoCoatMid, hoOpeningRegion, &hoEdgeRegion);
		ReduceDomain(m_hoAdaptedImage, hoEdgeRegion, &hoReduceDomain);
		Threshold(hoReduceDomain, &hoRegion, 0, hv_dThresh);
		OpeningRectangle1(hoRegion, &hoRegion, 3, 3);
		Connection(hoRegion, &hoRegion);
		ClosingRectangle1(hoRegion, &hoRegion, 15, 15);
		SelectShape(hoRegion, &hoSelectRegions, HTuple("width").Append("height").Append("area"), "and", hv_dMinWidth.TupleConcat(hv_dMinHeight).TupleConcat(hv_dMinArea), (HTuple(9999999).Append(9999999).Append(hv_dMaxArea)));
		if (hoSelectRegions.CountObj() > 0)
		{
			AddError(hoSelectRegions, NG_BlackSpot);
		}
	}
	catch (HException& e)
	{
...
{
	return (m_ptOut->hoDisp.CountObj() > 0);
}
