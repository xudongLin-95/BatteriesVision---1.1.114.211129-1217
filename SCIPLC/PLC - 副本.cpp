...
BOOL CPLC::KnifeCorrecting(C_MergeWork &Output)
{
	...
	try
	{
		HTuple hvLeftCorrectValue, hvRightCorrectValue;
		...
		if (TRUE == Output.vecB[0].tMeasure.hv_HasCoat.I())
		{
			...
			s_hvFilterData[LEFT_KNIFE].Append(hvLeftCorrectValue.D());
			...
			s_dData[LEFT_KNIFE] = hvData[LEFT_KNIFE].TupleMedian().D();//求刀量均值
			...
			double dDiffTmp = s_dData[LEFT_KNIFE] * m_tPLCSet.dKnifeCorrectPercent;//刀量*单次刀距闭环比例
