...
public:
	...
	typedef struct tagTPLCSET
	{
		...
		int nProductTypeAddress;//品种号写入地址
		int nPlcSoftwareVersionAddress;//软件版本号写入地址
		int nReadProductTypeAddress;//品种号读取地址

		BOOL bJudgeGuidFinish;//是否判断纠偏执行到位
		int nGuidFinishAddr;//判断纠偏执行到位的地址
		//刀距闭环地址
		int nUseKnifeCorrect;//使用使用纠偏
		int nPlcLeftKnifeCorrectAddress;//PLC设备左刀闭环地址
		int nPlcRightKnifeCorrectAddress;//PLC设备右刀闭环地址
		int nPlcKnifeDoCorrectAddress;//PLC设备执行刀距闭环地址
		BOOL bJudgeKnifeDoFinish;//是否判断刀执行到位
		int nPlcKnifeDoFinishAddress;//判断纠偏执行到位的地址
		int nPlcLeftKnifePosAddress;//PLC设备左刀实际位置地址
		int nPlcRightKnifePosAddress;//PLC设备右刀实际位置地址

		//模头移动闭环参数
		double dKnifeStartIngoreDistance;//开机忽略米数
		...
		int	nUseWeightData;//使用重量数据
		double dBetaRayDistance;//到测厚仪距离(m)
		double dBetaRayWeightDiffLimit;//重量数据差值上限，超过上限不执行闭环动作
		
		//刀距闭环参数
		double dKnifeAlertMaxDiff;//最大进退刀量，单位刀
		double dKnifeAlertMinDiff;//最小进退刀量，单位刀
		double dUnitKnifeCoatWidth;//单刀对应涂膜尺寸变化
		double dKnifeCorrectPercent;//单次刀距闭环比例
		double dCamKnifeDistance;//刀距到相机延迟距离
		double dKnifeCorrectInterval;//刀距闭环间距
...
public:
	...
	double m_dImageHeightMM;//每张图像的高度
private:
	PLCINIT m_sInit;
	...
	HTuple m_hvKnifeDiffValue;//刀距差值，单位刀，0左刀，1右刀
	HTuple m_hvKnifeCorrectValue;//刀距调整闭环值，单位刀，0左刀，1右刀

	double m_dCurrentLeftKnifePos;//左刀位置
	double m_dCurrentRightKnifePos;//右刀位置
private:
	//PLC线程
	...
	//刀距闭环逻辑
	BOOL KnifeCorrecting(C_MergeWork &Output);
	...
	//获取PLC左刀位置
	int GetPLCLeftKnifePosition(double& dPos);
	//获取PLC右刀位置
	int GetPLCRightKnifePosition(double& dPos);
	...
	//写入品种号，供喷码系统监控
	BOOL WriteProductTypeVerify(CString& strProductType);
	//获取品种号，供喷码系统监控
	BOOL GetProductTypeName(CString& strProductType);
	//写入软件版本号，供喷码系统监控
	BOOL WriteSoftwareVersionVerify(CString& strSoftwareVersion);
	...
	//发送进退刀闭环值
	BOOL _SendKnifeCorrectValue(double& dLeftValue, double& dRightValue);

	//判断模头是否移动到位
	BOOL _JudgeGuideFinish();

	//判断刀距是否移动到位
	BOOL _JudgeKnifeDoFinish();

	//判断刀距闭环的时候，重量数据是否有问题
	BOOL _JudgeKnifeWeightOK();

	//计算模头闭环纠偏值，包含可选择单双面，均值和标准差
	BOOL _CalCorrectDiff(C_MergeWork &Output, HTuple& hvDiff);

	//计算刀距闭环值
	BOOL _CalKnifeCorrectDiff(C_MergeWork &Output, HTuple& hvLeftDiff, HTuple& hvRightDiff);
