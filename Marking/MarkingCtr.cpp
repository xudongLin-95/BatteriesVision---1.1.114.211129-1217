...
#pragma region "创建队列 发送NG结果位置的queue"
		...
		SetMessageQueueParam(m_hvDefectToMarkigQueue, "max_message_num", 5);
...
int CMarkingCtr::PushMessage(const HTuple& hvMessageSend)//放置消息进入队列
{
	...
	catch (HException& e)
	{
		...
		{
			SetMessageQueueParam(m_hvDefectToMarkigQueue, "flush_queue", 1);
			CString strException;
			strException.Format("Marking队列已满,已清空队列");
			Log(strException, ExcpLog);
		}
		else
		{
...
//获取尺寸NG相关数据
BOOL CMarkingCtr::GetDefectSize(C_MergeWork& Result, int SideSN, vector<PARTITIONMSG>& vPartitionMsg, vector<tagDefect>& mtagDefects)
{
	try
	{
		...
		for (int i = 0; i < Result.vecB.size(); i++)
		{
			...
			for (int j = 0; j < Result.vecB[i].tMeasure.tCoat.hv_Width.Length(); j++)//单张图的涂布数
			{
			...
			if (num > Result.vecB[i].tMeasure.tCoat.hv_Width.Length() / 2)//未涂大于一半涂布数量
			{
				if (i == 0)
					checkSideA = false;
				else if (i == 1)
		...
		GetImageSize(Result.vecB[SideSN].tMeasure.ho_DispImage, &hv_wtdth, &hv_height);
		int nHeight = hv_height.I() * t_CameraBaseMsg.dVResolutionSideA;
		m_tSocketSet.nDivImageNum = (int)(nHeight / unitHeight) + ((nHeight%unitHeight) > 0 ? 1 : 0);//计算出垂直划分区域数

		//*重新汇总所有区域的尺寸NG类型，统一处理。一出三暂时没考虑
		...
		for (int i = 0; i < len; i++)
		{
			...
			if (!m_sInit.tPdInfo.tParam.bUseFoilWidth && (stripSN % 4 == 0))//极耳分条
				continue;
			else if (!m_sInit.tPdInfo.tParam.bUseCoatWidth && (stripSN == 2 || stripSN == 6 || stripSN == 10 || stripSN == 14 || stripSN == 18))//涂布分条
				continue;
			else if (!(m_sInit.tPdInfo.bExistBaseCoat || m_sInit.tPdInfo.bExistAT11) && !(m_sInit.tPdInfo.tParam.bUseBaseCoatWidth || m_sInit.tPdInfo.tParam.bUseAT11Width) && (stripSN % 2 != 0))//凹版、AT9/AT11分条（有凹版的情况下）
				continue;
			int type = Result.tMerge.hv_NGType2[i].I();//选择类型，这里面只包括尺寸NG类型
			//
			//int stripSN = Result.vecB[SideSN].tMeasure.tDefect.hv_StripeSN[i].I();//分条号
			//有凹版、AT9/AT11等
			if (m_sInit.tPdInfo.tParam.bUseFoilWidth && (m_sInit.tPdInfo.bExistBaseCoat || m_sInit.tPdInfo.bExistAT11) && (stripSN % 4) == 0)//极耳
			{
				HTuple foilRow1, foilCol1, foilRow2, foilCol2;
				if (Result.vecB[SideSN].tMeasure.tFoil.ho_Region.CountObj() > 0)//检测图所有极耳尺寸和区域
				{
					HObject hv_sortRegion;
					SortRegion(Result.vecB[sideSN].tMeasure.tFoil.ho_Region, &hv_sortRegion, "first_point", "true", "column");
					//如果region坐标是乱序，则先将region排序
					SmallestRectangle1(hv_sortRegion, &foilRow1, &foilCol1, &foilRow2, &foilCol2);//所有极耳区域生成最小外接矩形，并给出坐标数组
				}
				//根据分条号计算出对应极耳区域
				int foilSN = stripSN / 4;
				HTuple foilRealCol = foilCol1[foilSN];//左上列坐标
				HTuple foilRealRow1 = foilRow1[foilSN];//左上行坐标
				HTuple foilRealCo2 = foilCol2[foilSN];//右下列坐标
				double foilRealRow2 = foilRow2[foilSN].D();//左上行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[i].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= foilRealRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[i].nRow1.push_back(foilRealRow1);
						mtagDefects[i].nRow2.push_back(foilRow2[foilSN].D()*t_CameraBaseMsg.dVResolutionSideA);
					}
					else
					{
						mtagDefects[i].nRow1.push_back(foilRealRow1);
						mtagDefects[i].nRow2.push_back((nDivNum + 1)*m_tSocketSet.nMarkingHeight);
					}
					foilRealRow1 = (nDivNum + 1)*m_tSocketSet.nMarkingHeight;
					//转换像素坐标至物理坐标
					HTuple hvBaseSizeCol1 = (Result.vecB[SideSN].tMeasure.tLine.hv_StartColumn[0].D() + Result.vecB[SideSN].tMeasure.tLine.hv_EndColumn[0].D()) / 2;
					HTuple hvRealX, hvBaseSizeCol;
					int nSideSN = Result.tMerge.hv_NGSideSN[i].I(); //NG面序号
					CString strSide;
					strSide.Format("Side%d", nSideSN);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &hvBaseSizeCol1, &hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &foilRealCol, &hvRealX);
					mtagDefects[i].nCol1.push_back(hvRealX - hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &foilRealCo2, &hvRealX);
					mtagDefects[i].nCol2.push_back(hvRealX - hvBaseSizeCol);
					/*mtagDefects[i].nCol1.push_back(foilRealCol);
					mtagDefects[i].nCol2.push_back(foilRealCo2);*/

				}

			}

			else if (m_sInit.tPdInfo.tParam.bUseFoilWidth && (m_sInit.tPdInfo.bExistBaseCoat || m_sInit.tPdInfo.bExistAT11) && (stripSN % 2) == 0)//涂膜区域
			{
				HTuple CoatRow1, CoatCol1, CoatRow2, CoatCol2;
				if (Result.vecB[SideSN].tMeasure.tCoat.ho_Region.CountObj() > 0)//检测图所有涂膜尺寸和区域
				{
					/*HObject hv_sortRegion;
					SortRegion(Result.vecB[sideSN].tMeasure.tFoil.ho_Region, &hv_sortRegion, "first_point", "true","column");*/

					//如果region坐标是乱序，则先将region排序
					SmallestRectangle1(Result.vecB[SideSN].tMeasure.tCoat.ho_Region, &CoatRow1, &CoatCol1, &CoatRow2, &CoatCol2);//所有极耳区域生成最小外接矩形，并给出坐标数组
				}
				//根据分条号计算出对应极耳区域

				int foilSN = stripSN / 4;
				HTuple coatPixCol = CoatCol1[foilSN];//左上列坐标
				HTuple coatPixRow1 = CoatRow1[foilSN];//左上行坐标
				HTuple coatPixCo2 = CoatCol2[foilSN];//右下列坐标
				double coatPixRow2 = CoatRow2[foilSN];//右下行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[i].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= coatPixRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[i].nRow1.push_back(coatPixRow1);
						mtagDefects[i].nRow2.push_back(CoatRow2[foilSN] * t_CameraBaseMsg.dVResolutionSideA);
					}
					else
					{
						mtagDefects[i].nRow1.push_back(coatPixRow1);
					...
					coatPixRow1 = (nDivNum + 1)*m_tSocketSet.nMarkingHeight;
					//转换像素坐标至物理坐标
					HTuple hvBaseSizeCol1 = (Result.vecB[SideSN].tMeasure.tLine.hv_StartColumn[0].D() + Result.vecB[SideSN].tMeasure.tLine.hv_EndColumn[0].D()) / 2;
					HTuple hvRealX, hvBaseSizeCol;
					int nSideSN = Result.tMerge.hv_NGSideSN[i].I(); //NG面序号
					...
					strSide.Format("Side%d", nSideSN);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &hvBaseSizeCol1, &hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &coatPixCol, &hvRealX);
					mtagDefects[i].nCol1.push_back(hvRealX - hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &coatPixCo2, &hvRealX);
					mtagDefects[i].nCol2.push_back(hvRealX - hvBaseSizeCol);
			...
			else if (m_sInit.tPdInfo.tParam.bUseFoilWidth && m_sInit.tPdInfo.bExistBaseCoat && !m_sInit.tPdInfo.bExistAT11 && (stripSN % 2) != 0)//所有底涂/凹版/涂碳 尺寸和区域
			{
				HTuple BaseCoatRow1, BaseCoatCol1, BaseCoatRow2, BaseCoatCol2;
				if (Result.vecB[SideSN].tMeasure.tBaseCoat.ho_Region.CountObj() > 0)//检测图所有涂膜尺寸和区域
				{
					/*HObject hv_sortRegion;
					SortRegion(Result.vecB[sideSN].tMeasure.tFoil.ho_Region, &hv_sortRegion, "first_point", "true","column");*/
					//如果region坐标是乱序，则先将region排序
					SmallestRectangle1(Result.vecB[SideSN].tMeasure.tBaseCoat.ho_Region, &BaseCoatRow1, &BaseCoatCol1, &BaseCoatRow2, &BaseCoatCol2);//所有极耳区域生成最小外接矩形，并给出坐标数组
				}
				//根据分条号计算出对应极耳区域

				int BaseCoatSN = stripSN / 2;
				HTuple basecoatPixCol = BaseCoatCol1[BaseCoatSN];//左上列坐标
				HTuple basecoatPixRow1 = BaseCoatRow1[BaseCoatSN];//左上行坐标
				HTuple basecoatPixCo2 = BaseCoatCol2[BaseCoatSN];//右下列坐标
				double basecoatPixRow2 = BaseCoatRow2[BaseCoatSN].D();//右下行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[i].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= basecoatPixRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[i].nRow1.push_back(basecoatPixRow1);
						mtagDefects[i].nRow2.push_back(BaseCoatRow2[BaseCoatSN].D()*t_CameraBaseMsg.dVResolutionSideA);
					}
					else
					{
						mtagDefects[i].nRow1.push_back(basecoatPixRow1);
					...
					basecoatPixRow1 = (nDivNum + 1)*m_tSocketSet.nMarkingHeight;
					//转换像素坐标至物理坐标
					HTuple hvBaseSizeCol1 = (Result.vecB[SideSN].tMeasure.tLine.hv_StartColumn[0].D() + Result.vecB[SideSN].tMeasure.tLine.hv_EndColumn[0].D()) / 2;
					HTuple hvRealX, hvBaseSizeCol;
					int nSideSN = Result.tMerge.hv_NGSideSN[i].I(); //NG面序号
					...
					strSide.Format("Side%d", nSideSN);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &hvBaseSizeCol1, &hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &basecoatPixCol, &hvRealX);
					mtagDefects[i].nCol1.push_back(hvRealX - hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &basecoatPixCo2, &hvRealX);
					mtagDefects[i].nCol2.push_back(hvRealX - hvBaseSizeCol);
					/*mtagDefects[i].nCol1.push_back(basecoatPixCol);
					mtagDefects[i].nCol2.push_back(basecoatPixCo2);*/

				}

			}

			else if (m_sInit.tPdInfo.tParam.bUseFoilWidth && !m_sInit.tPdInfo.bExistBaseCoat && m_sInit.tPdInfo.bExistAT11 && (stripSN % 2) != 0)//所有陶瓷/AT9/AT11 尺寸和区域
			{
				HTuple AT11Row1, AT11Col1, AT11Row2, AT11Col2;
				if (Result.vecB[SideSN].tMeasure.tAT11.ho_Region.CountObj() > 0)//检测图所有涂膜尺寸和区域
				{
					/*HObject hv_sortRegion;
					SortRegion(Result.vecB[sideSN].tMeasure.tFoil.ho_Region, &hv_sortRegion, "first_point", "true","column");*/
					//如果region坐标是乱序，则先将region排序
					SmallestRectangle1(Result.vecB[SideSN].tMeasure.tAT11.ho_Region, &AT11Row1, &AT11Col1, &AT11Row2, &AT11Col2);//所有极耳区域生成最小外接矩形，并给出坐标数组
				}
				//根据分条号计算出对应极耳区域

				int AT11SN = stripSN / 2;
				HTuple at11PixCol = AT11Col1[AT11SN];//左上列坐标
				HTuple at11PixRow1 = AT11Row1[AT11SN];//左上行坐标
				HTuple at11PixCo2 = AT11Col2[AT11SN];//右下列坐标
				double at11PixRow2 = AT11Row2[AT11SN].D();//右下行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[i].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= at11PixRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[i].nRow1.push_back(at11PixRow1);
						mtagDefects[i].nRow2.push_back(AT11Row2[AT11SN].D()*t_CameraBaseMsg.dVResolutionSideA);
					}
					else
					{
						mtagDefects[i].nRow1.push_back(at11PixRow1);
					...
					at11PixRow1 = (nDivNum + 1)*m_tSocketSet.nMarkingHeight;
					//转换像素坐标至物理坐标
					HTuple hvBaseSizeCol1 = (Result.vecB[SideSN].tMeasure.tLine.hv_StartColumn[0].D() + Result.vecB[SideSN].tMeasure.tLine.hv_EndColumn[0].D()) / 2;
					HTuple hvRealX, hvBaseSizeCol;
					int nSideSN = Result.tMerge.hv_NGSideSN[i].I(); //NG面序号
					...
					strSide.Format("Side%d", nSideSN);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &hvBaseSizeCol1, &hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &at11PixCol, &hvRealX);
					mtagDefects[i].nCol1.push_back(hvRealX - hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &at11PixCo2, &hvRealX);
					mtagDefects[i].nCol2.push_back(hvRealX - hvBaseSizeCol);
					/*mtagDefects[i].nCol1.push_back(at11PixCol);
					mtagDefects[i].nCol2.push_back(at11PixCo2);*/

				}

			}
			//无凹版、AT9/AT11
			else if (m_sInit.tPdInfo.tParam.bUseFoilWidth && !(m_sInit.tPdInfo.bExistBaseCoat || m_sInit.tPdInfo.bExistAT11) && (stripSN % 2) == 0)//极耳
			{
				HTuple foilRow1, foilCol1, foilRow2, foilCol2;
				if (Result.vecB[SideSN].tMeasure.tFoil.ho_Region.CountObj() > 0)//检测图所有极耳尺寸和区域
				{
					...
					//如果region坐标是乱序，则先将region排序
					SmallestRectangle1(Result.vecB[SideSN].tMeasure.tFoil.ho_Region, &foilRow1, &foilCol1, &foilRow2, &foilCol2);//所有极耳区域生成最小外接矩形，并给出坐标数组
				}
				//根据分条号计算出对应极耳区域
				int foilSN = stripSN / 2;
				HTuple foilRealCol = foilCol1[foilSN];//左上列坐标
				HTuple foilRealRow1 = foilRow1[foilSN];//左上行坐标
				HTuple foilRealCo2 = foilCol2[foilSN];//右下列坐标
				double foilRealRow2 = foilRow2[foilSN].D();//右下行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[sideSN].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= foilRealRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[sideSN].nRow1.push_back(foilRealRow1);
						mtagDefects[sideSN].nRow2.push_back(foilRow2[foilSN].D()*t_CameraBaseMsg.dVResolutionSideA);
					}
					else
					{
						mtagDefects[sideSN].nRow1.push_back(foilRealRow1);
						mtagDefects[sideSN].nRow2.push_back((nDivNum + 1)*m_tSocketSet.nMarkingHeight);
					}
					foilRealRow1 = (nDivNum + 1)*m_tSocketSet.nMarkingHeight;
					//转换像素坐标至物理坐标
					HTuple hvBaseSizeCol1 = (Result.vecB[SideSN].tMeasure.tLine.hv_StartColumn[0].D() + Result.vecB[SideSN].tMeasure.tLine.hv_EndColumn[0].D()) / 2;
					...
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &hvBaseSizeCol1, &hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &foilRealCol, &hvRealX);
					mtagDefects[sideSN].nCol1.push_back(hvRealX - hvBaseSizeCol);
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &foilRealCo2, &hvRealX);
					mtagDefects[sideSN].nCol2.push_back(hvRealX - hvBaseSizeCol);
					/*mtagDefects[i].nCol1.push_back(foilRealCol);
					mtagDefects[i].nCol2.push_back(foilRealCo2);*/
			...
			else if (m_sInit.tPdInfo.tParam.bUseFoilWidth && !(m_sInit.tPdInfo.bExistBaseCoat || m_sInit.tPdInfo.bExistAT11) && (stripSN % 2) != 0)//涂膜区域
			{
				...
				HTuple coatPixRow1 = CoatRow1[foilSN];//左上行坐标
				HTuple coatPixCo2 = CoatCol2[foilSN];//右下列坐标
				double coatPixRow2 = CoatRow2[foilSN].D();//右下行坐标
				double coatPixRow01 = CoatRow1[foilSN];//左上行坐标
				for (int nDivNum = 0; nDivNum < m_tSocketSet.nDivImageNum; nDivNum++)
				{
					mtagDefects[sideSN].nType.push_back(type);
					if ((nDivNum + 1)*m_tSocketSet.nMarkingHeight >= coatPixRow2*t_CameraBaseMsg.dVResolutionSideA)
					{
						mtagDefects[sideSN].nRow1.push_back(coatPixRow1);
						mtagDefects[sideSN].nRow2.push_back(CoatRow2[foilSN].D()* t_CameraBaseMsg.dVResolutionSideA);
						double x_1 = CoatRow2[foilSN] * t_CameraBaseMsg.dVResolutionSideA;
					}
					else
					{
						mtagDefects[sideSN].nRow1.push_back(coatPixRow1);
					...
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &coatPixCol, &hvRealX);
					mtagDefects[sideSN].nCol1.push_back(hvRealX - hvBaseSizeCol);
					double hvbasecol = hvBaseSizeCol;
					double y01 = coatPixCol;
					double y001 = hvRealX;
					double y0001 = hvRealX - hvBaseSizeCol;
					m_sInit.Main(MT_Marking, "CalColPoint", strSide, &coatPixCo2, &hvRealX);
					double y02 = coatPixCo2;
					double y002 = hvRealX;
					double y0002 = hvRealX - hvBaseSizeCol;
					mtagDefects[sideSN].nCol2.push_back(hvRealX - hvBaseSizeCol);
					/*mtagDefects[i].nCol1.push_back(coatPixCol);
					mtagDefects[i].nCol2.push_back(coatPixCo2);*/
...
BOOL CMarkingCtr::SendResultToMarking(C_MergeWork& Result)
{
	try
	{
		...
		if (m_tSocketSet.nUseCheckSize)
		{
			...
			{
				//*获取尺寸结果信息
				GetDefectSize(Result, i, m_vPartitionMsg, m_tagDefects);
			}
			for (int num = 0; num < m_tagDefects.size(); num++)
			{
				count += m_tagDefects[num].nCol1.size();
...
void CMarkingCtr::marking_thread()
{
	...
	while (TRUE)
	{
		...
		try
		{
			...
			//if (nOldIOCount != IONo)
			{
				if (m_vMarkingRes.size() == m_tSocketSet.nImgDiff + 1)
				{
					nOldIOCount = IONo;
					m_nIOCount++;
					...
					{
						CString str = tMarkingRes.sResult;
				...
				else if (m_vMarkingRes.size() > m_tSocketSet.nImgDiff + 1)
				{
