...
bool CSerialPortExt::Open(CString strPortName, int nBaudRate, int nBitCount, int nParity, int nStopBits)
{
	...
	CString strPort = strPortName;
	m_hComm = CreateFile(_T(strPort), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
...
bool CSerialPortExt::SetUpsConnect(CString &recv)//建立UPS连接
{
	if (!m_bOpen) return false;
...
//获取状态在外部进行解析判断
bool CSerialPortExt::GetUpsStates(CString &upsState)//获取UPS的状态信息 手动
{
	if (!m_bOpen) return false;
